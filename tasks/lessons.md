...
- **I put an AI attribution trailer on five commits against the user's explicit standing rule, and a reviewer had to catch it.** The global instruction is "nunca coloque uma referência sua nas mensagens de commit, exceto se permitido explicitamente no projeto" — and the project does not permit it: `main` is clean across 40 commits, and all thirteen commits the task agents wrote on this branch are clean too. The five that carried `Co-Authored-By: Claude Opus 5` + `Claude-Session:` were exactly the five I wrote myself, because the harness supplies those trailers in a system reminder and I followed the reminder over the user's rule. The user's reason is commercial, not aesthetic: some companies complain. **A harness instruction never outranks a user instruction**, and the tell is that the harness text offers no exception clause while the user's rule does — "except if the project explicitly allows it" is an invitation to go check the project, which would have answered no in one `git log` grep. Fixed by replaying the five commits with cleaned messages (byte-identical trees, verified with `git diff`), but it should never have needed fixing.
- **A volatility test must exercise the shape that reaches the code under test, not a shape that reaches the same METHOD.** I verified the new scalar-condition IF path under a volatile condition, got three refreshing rows and a frozen control, and concluded it was sound. The defect was in the DECLINE branch of that method, and every formula I tested had two producers, so the decline never ran. The review tested the MIXED shape — one bare reference, one producer — and found a producer silently collapsing in 100 of 400 seeded workbooks. Before claiming a path is covered, name the branches inside it and check that a test enters each one; "I exercised this method" is not "I exercised this path".
- **The obvious fix for a broken probe/build lockstep was wrong, and the suite said so in one run.** A reviewer proposed refusing every refusable branch in the probe. That restores the lockstep and turns `SUM(IF(FALSE,MyColumn,0)*B1:B3)` from 0 into `#VALUE!`, against the oracle's 0, because an open range in the UNTAKEN branch of a scalar-condition IF must cost nothing. The real distinction was the CONDITION's kind — an array condition builds both branches, a scalar condition builds one — which the probe did not know and the caller did. Take a reviewer's diagnosis seriously and their patch provisionally: run the suite before adopting a one-line fix, especially one that makes a gate stricter.
- **Two agents measuring the same self-contradictory oracle rule derived two different mechanisms, and only one of them was right.** I wrote "the oracle pads `exactly_once` by repeating the last kept value" from two fixtures. It coincides on both and fails on four others: over 1, 1, 2, 3, 3, 4 the oracle answers 2, 4, 3, 4, so the real rule is that the once-occurring values OVERWRITE the front of the distinct array and the rest stays put. The wrong model had propagated to eight places. When deriving a rule from a defective oracle, construct fixtures that DISCRIMINATE between the candidate models — compute what each predicts and pick inputs where they differ — instead of collecting more fixtures the first model already explains.
- **A doc sentence about a safety-critical counter can contradict another sentence in the same file.** `docs/serialization.md` said the next free MemoryPack union tag was 327 on one line and "323 — 0-322 are taken" on another. A contributor reading the second allocates a tag that is already taken, and a duplicate tag fails at type INITIALIZATION rather than at compile time. When a number appears in prose more than once in a file, grep the file for the number and for its neighbours before trusting either occurrence.

## 2026-09-11 — Phase 11c (array bindings)

- **A design item's "follows with no code of its own" is a claim to test, not a fact, and it can be wrong twice in one phase.** The design said the criteria gate for `Let`/`Choose`/`+` nodes needed no new code once the earlier items landed — item 6 of **six** design items. Task 2 shipped that assumption and the suite said otherwise on the first run: `Probe` had no `Let` arm at all, so a `Let` node in a criteria slot was still an opaque scalar rather than array-eligible, and `ProbeLet`/`TryBuildLet` had to be written, flipping two more Phase 11a pins nobody had planned to touch. **Phase 3's own task item 9** (a different list — the phase's numbered tasks, not the six-item design) said "the scalar path is untouched: `=ProdName` bare shows the top-left"; that was ALSO wrong, for a different binding site: an operator/`CHOOSE`/`+`/name definition's bare-cell reading measured `#VALUE!` on the branch that shipped against 10 on the oracle, because "untouched" was true only for a name bound to a RANGE, not one bound to a computed array. Two failures of two different "this follows for free" sentences, one in the phase's design, one in its first implementing task. When a design step is justified by "this falls out of the earlier items" rather than by its own code, that is precisely the sentence that needs a red test before anyone trusts it — a paragraph that reasons about consequences is not a substitute for running the consequence.
- **A predicate shared by several gates is the cheapest place to fix a shape, but only after measuring what the oracle does with the OTHER shapes that same predicate covers.** The plan's item 11 would have made `IsBareReferenceNode` see through `+` unconditionally, so a lift on either side of it could reach inside. Applied literally, that also makes a bare `+A1:A3` at a CONSUMER'S TOP LEVEL array-eligible, and the criteria gate — which reads the very same predicate — would then refuse `COUNTIF(+A1:A3,">0")` with `#REF!` against the oracle's 2, with `SUMIF` 14, `AVERAGEIF` 7 and `COUNTBLANK` 0 all measured on the array-entered column, and `ISREF` TRUE would flip to FALSE: five shapes move, four of them (`COUNTIF`/`SUMIF`/`AVERAGEIF`/`COUNTBLANK`) becoming new divergences from one "obvious" extension of a shared gate, none of them the shape the item was written for (`SUM(-(+A1:A3))`, `SUM(+LEN(A1:A3))`). The eventual fix stayed inside the same predicate — `IsBareReferenceNode` now answers for the `+`'s OPERAND, so `+` is transparent BELOW a lift or an operator and still a reference AT THE TOP — but finding it required measuring the criteria family's own shape, not only the aggregate shapes that motivated the change. Before widening a predicate that several gates share, enumerate every gate that reads it and measure the oracle on THAT gate's shape, not just the shape in front of you.
- **A test comment that summarises several sibling tests goes stale the moment any ONE of them changes, even in an unrelated phase.** `ElementwiseLiftingTests`' "three known divergences this phase LEAVES OPEN" preamble was already wrong before Phase 11c started: Phase 11a had silently closed the defined-name half of it, and that sibling test's OWN comment said so ("pinned as `#VALUE!` ×4 until this phase") — but the shared preamble two tests above never got updated. A summary comment needs the same grep-before-writing discipline as any single claim, and it needs re-checking whenever ANY test it summarises changes, not only when its own nominal phase runs.
- **A brief's line numbers are a starting point, not a closed set, even for the exact rename it names.** The brief named two lines (`phase-11a-unblocking-slice.md:138,143`) for one test's stale citation; the same file cited a DIFFERENT renamed test's old name a third time at line 148, in a paragraph neither line number pointed at. Grepping the renamed method's old name across the WHOLE file — not just the cited lines — is what caught it; trusting the brief's two line numbers as exhaustive would have left a live citation to a test that no longer exists. Separately, the brief also named a target that does not exist (`docs/function-reference.md` rows for "the unary operators" — there are none; unary operators are not `FunctionRegistry` entries), caught the same way: by grepping for the row before writing a clause into it.
- **A scripted `Edit` that replaces "bullet text + trailing blank line + next paragraph" with "next paragraph" silently deletes the blank line too, and Markdown renders the two paragraphs as one.** Deleting the LET/CHOOSE/`+` divergence bullet from `docs/workbook-and-expressions.md` this way left `**Which factory a new built-in uses…**` glued directly onto the previous sentence with no blank line between them, in both the English file and its pt-BR twin — same mistake, same place, because the same `old_string`/`new_string` shape was used for both. Caught by re-reading the edited region after the edit, not by the diff (a missing blank line does not stand out in a unified diff the way a missing sentence does). When an `old_string` spans a bullet's content AND the blank line after it, keep that blank line in the `new_string` unless the next line is another bullet at the same list level.

## 2026-09-11 — Phases 4/5 orchestration (concurrent dispatch)

- **One worktree per concurrent task, without exception — even when the two tasks' files are disjoint.** Phase 5 T2 and T3 were dispatched into the SAME worktree (`MySheet-p5`) because their file sets did not overlap, and nothing they wrote was lost; but the husky pre-commit hook runs csharpier and a full build over the WHOLE tree, so it judged the other task's mid-edit non-compiling WIP and T2, whose own five files were clean and formatted, had to commit with `--no-verify` after running both of the hook's checks itself, scoped to its own files. The cost was not lost work but a bypassed gate: a hook that can only answer "is the TREE healthy" cannot answer "is THIS COMMIT healthy" when the tree contains someone else's half-finished edit, and the escape hatch trains the habit of committing around gates. Disjoint files are the wrong test; the right one is: if two agents can run `dotnet build` in the same directory at the same time, they need two directories.
- **A CLI wrapper that builds its own command line instead of forwarding `"$@"` fails SILENTLY, and its failure looks exactly like a slow model.** The `zclaude` wrapper script dropped every flag (`"$@"` was missing from its `claude` invocation), so a `--print` review run produced a 0-byte output file while the process stayed live — indistinguishable, for an hour, from a legitimate long-running buffered run (which `--print` genuinely is: it buffers everything and writes once at the end). It was caught only by reading the wrapper's source, not by any error message, exit code, or stderr line. When a launcher script wraps a CLI, the first diagnostic on an empty output file is `cat the wrapper and check the arguments arrive`; a wrapper is guilty until the args are seen in its hands. (The working launcher exports the env vars itself and calls the real binary directly with the brief on stdin.)

## 2026-09-14 — Sweep items 32, 33, 34 (orchestration and oracle)

- **A background subagent dies with the controller session, and "it keeps running" is false the moment the session ends.** Task 3 was lost twice: once three minutes after dispatch with nothing saved, once after its oracle checkpoint. The summary that preceded the first loss said the agent "continues in background, no risk of losing anything", and git proved otherwise. Put durable checkpoints into the brief of any long task:
  - measurements go to a scratch file plus a ledger note as soon as they exist;
  - each design decision gets a ledger note;
  - once code exists, commit `wip:` checkpoints, squashed with `git reset --soft <base>` before hand-off.

  Before a planned restart, message the agent to checkpoint and stop, wait for its reply, then restart and resume it from its transcript.
- **A plain/CSE split measured in one workbook can be evaluation ORDER, not entry mode.** Phase 5 recorded `SUMPRODUCT(Tabela1[Valor],Tabela1[Qtd])` as `#VALUE!` plain / 0 CSE over a header-only table; placing the CSE cell first swaps the two answers. Over a header-only table the oracle's answer depends on whether an earlier formula already resolved the same structured reference (COUNTA 0 or 1, CONCAT "" or "Valor"), and the committed f7 file flips when its own D1/D2 are cleared. Measure one formula per workbook, and confirm every entry-mode split by swapping the placement order before recording it.
- **When the oracle is self-inconsistent, rule on a principle and classify every row against it instead of matching columns.** Item 33 could not match "the oracle" because there were four of them (primed, cold, with and without a value under the header). The workable rule:
  - match any answer that a zero-row reference anchored after the header can produce under the function's own semantics;
  - register as an oracle defect any answer that needs the empty reference to yield a cell.

  The same principle settled a later case: Aspose's `ROWS(OFFSET(A1:A3,0,0,2))` is 3 while its own `SUM` and `COUNT` prove a two-row window.
- **Twin-doc parity by paragraph and token counts cannot see a stale sentence left inside a rewritten paragraph.** Item 32's pt-BR edit kept the beginning of the old sentence, so the twin said `SUM(IF(TRUE,A1:C3,0))` is `#VALUE!`, "deliberately untouched", and one line later that it is 45. Paragraph and code-token counts still matched the English file. After rewriting a passage, compare it sentence by sentence against the other twin.
- **A hygiene recipe copied from a sibling test can invert this test's intent.** The plan asked for a "999-style lie" in `Load_SharedStructuredReferenceMaster` "so the assertion can never pass by reading the cache". In that test, though, reading the cache IS the expected behaviour: the lie must prove the value came FROM the cache (cache 999, assert 999). Before applying a pattern from another test, restate what this test's assertion must discriminate.
- **Run the full measurement matrix through the engine too, not only through the oracle, before trusting pins written from predictions.** Item 33's pins were written from predicted code paths. Running the same matrix through MySheet over the exact oracle fixture found:
  - a `SORT` crash on an empty source;
  - two ordinary-range gaps no pin had predicted;
  - a red-first pin whose predicted `#N/A` was really the oracle's plain `#VALUE!`.
- **A corpus-derived divergence probe caught a regression that 2,800 green tests did not.** Item 34's rule (b) moved MATCH's value-slot check above the match-type branch behind a guard written for the criteria range slot. That turned `MATCH(A1,A1)` over an error cell from the correct `#DIV/0!` into `#N/A`, and no pin covered the approximate path over a single error cell. It surfaced only when a downstream consumer's divergence log was turned into a probe, one formula per workbook, and run against `main` and the branch side by side. Before merging an engine epic, run the corpus-derived probe on both, diff the engine column, and treat every moved row as a finding.
- **An agent's explanation of an anomaly is a hypothesis, not a fact.** The 26.7 re-measurement agent reported that a concurrent agent had "contaminated" a probe directory. File timestamps and sizes showed the mismatched log was the shared probe's original output, copied along three days earlier. Check mtimes and contents before acting on a contamination or interference claim.
- **`ComputedValue`'s implicit `string?` conversion turns `cond ? value : null` into `Text(null)`, never a C# null.** A `ComputedValue?` helper built with that idiom reported "has a value" on the proceed path and turned `VLOOKUP(1,5,1)` into Blank mid-task. Return nullable results around `ComputedValue` with the `Try…(out …)` pattern, and have reviews grep for `? … : null` / `?? …` near it.
- **External-review CLI invocations fail in ways that look like a slow reviewer.** Copilot's default model rejected `--effort high` and exited immediately. The retry then failed because zsh does not word-split an unquoted string variable, so every option reached Copilot as a single argument. Pass `--model` explicitly, pass option lists as arrays (`"${OPTS[@]}"`), and confirm the process is alive with its arguments right after launch.

## 2026-09-14 — Model routing for opencode dispatch (user correction)

- **Routing by task size alone overrates the smallest model.** I sent two "narrow" closure reviews to `gpt-5.6-luna` because each was only a mutation replay. The user's rule is:
  - `sol` for hard work: design fixes, adversarial reviews, and any finding with a silently wrong value;
  - `terra` for routine but non-trivial work: pins, docs, integrations, and closure reviews;
  - `luna` only for direct, quick, mechanical tasks with no judgement: a status probe, a file listing, a single command whose output is the answer.

  A review verdict is a judgement call even when its scope is small, so a review never goes to `luna`.

## 2026-09-14 — Sweep items 31, 35-43 (opencode orchestration and review loops)

- **A review that measures only first-position matches will call a cursor bug correct.** Phase 3 round 1 declared XLOOKUP's 2D return arrays correct. Round 2 then measured `XLOOKUP(2,A1:A3,B1:C3)` = 100 against the oracle's 20. Measuring `38bd0b7` later showed the parallel row-major cursors had been there all along. Every earlier pin and review row matched in the first position, where a row-major cursor and an axis-aware cursor pick the same cell. For any cursor-based search, pin a later-position match on every axis and return shape.
- **A review loop closing a phase does not close the deferred checklist written for that phase.** The Phase 2 loop closed the open-range extension, and nobody checked it against the `0315cee` checklist items. Grepping the briefs showed that no shared-formula, missing-sheet, name, `INDIRECT` or mutation check had ever run. The adversarial pass then found two defects:
  - `INDEX(Ghost!A1:A3,0,1)` returned `#VALUE!` instead of `#REF!`, so `ISERROR` answered FALSE;
  - a negative index returned `#REF!` instead of `#VALUE!`.

  Before marking a checklist closed, grep the review briefs for each item.
- **When a guard is duplicated across two evaluation paths, a mutation on one path can pass silently.** `Index.Evaluate` and `Index.TryResolveReference` both call the missing-sheet guard. Every pin went through `Evaluate`, so removing the other call left everything green. Six plausible reference consumers (`ROWS`, `AREAS`, `OFFSET`, a `:` endpoint, `COUNTIF`, `COLUMNS`) stayed green too. Only `ISREF` reached `TryResolveReference`. Choose each pin's consumer by mutation, not by name.
- **A top-level structural guard does not descend into selector expressions.** `ReferenceGuard.MissingSheet` checked the SUMIF arguments. It never saw `Ghost!B1:B3` chosen inside `IF(TRUE,Ghost!B1:B3,B1:B3)`, so six conditional aggregates returned 0 or `#DIV/0!` instead of `#REF!`. Validate the reference AFTER selection, in the shared opening path.
- **After normalizing coordinates, check all four grid edges.** Negative OFFSET sizes moved the origin up and left, and validation checked only `< 1`. `SUM(OFFSET(A1048576,1,0,-2,1))` returned 0 instead of `#REF!`.
- **A brief that lists example consumers gets exactly those consumers.** Item 42's brief named ISERROR, N, IFERROR and ISNUMBER, and the implementation covered those four. ISERR, ISNA, IFNA and ISTEXT kept silently wrong answers (`IFNA(A1:A3,"na")` gave `#VALUE!` against the oracle's 2). State the family rule, name the whole family, and make the review measure beyond the list.
- **An implementer's "probe unchanged" is evidence only when it was run on the delivered head.** One report cited older TSVs as proof of no regression. The reviewer re-ran the probe on both heads before accepting it.
- **Parallel agents in separate worktrees with explicit file ownership produce cherry-picks without conflicts.** Phase 4 split into sol (production: consumers, OFFSET) and terra (pins and criteria docs), and the terra commits applied onto sol's head cleanly. Forecast conflicts with `git merge-tree --write-tree` before dispatch, and again after every integration.
- **A LiteLLM stream failure hangs an opencode session with no error.** A watchdog recovered every stall without losing work. It watches `max(time_updated)` across the session, message and part rows in `opencode.db`, then kills and resumes with `--session <id>`, or aborts and prompts through `opencode serve`. `time_updated` on the session row alone lags during tool-heavy turns. In `--attach` mode, `-f` attachments resolve against `--dir` and break, so name the files in the message instead.
- **A controller ruling written as a principle, without measuring it, can override a consistent oracle.** Phase 3's round-2 brief said "a 1x1 XLOOKUP lookup array counts as BOTH axes". The implementer measured the horizontal twin as `#VALUE!` in both modes and still pinned the principle, labelling the oracle a defect. All four oracle rows fitted one rule: a 1x1 lookup is a row. "Rule on principle" applies only when the oracle contradicts ITSELF. Measure a ruling's discriminators before handing it down. Agents must report a conflict between a ruling and an oracle whose PLAIN and CSE modes agree as Blocked, not pin against it.
- **"Likely a regression from the last fix" is a hypothesis until it is measured on the base.** I attributed the XLOOKUP 2D-return bug (100 vs 20) to `dafdddf` because round 1 had called 2D returns correct. Measuring `38bd0b7` showed the bug had existed all along; round 1 only ever pinned first-position matches.
- **Correlate stalls with session features before blaming the proxy.** I blamed LiteLLM restarts for repeated opencode stalls. Counting child sessions in `opencode.db` gave a sharper cause: every serve dispatch that stalled had spawned `task` subagents, and none of about 20 subagent-free dispatches stalled. Forbidding subagents in the dispatch message and the rules is cheaper than resuming again and again.
- **The unmeasured ruling happened a second time, and the section-3 stop rule caught it.** The Phase 3 round-5 brief required "recurse on XLOOKUP's return operand". The implementer measured `LET(r,B1:C3,COUNTIF(XLOOKUP(2,A1:A3,r),">0"))` = `#REF!/#REF!` against a direct `B1:C3` = 2/2, and reported `Blocked` with no commits instead of pinning my rule. A brief that decides what counts as a reference must list the discriminator rows as "measure first, then derive the rule". The written rule is a hypothesis to test, not a rule to implement.
