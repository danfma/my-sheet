...
- [ ] **B2. Item 8's MissingSheet outcome does not broadcast a `#REF!` — a ghost-bound name evaluates to a REFERENCE value, and the "silent 1" survives the fix.** The item says `MissingSheet → (true, false)` "so the `#REF!` is what gets broadcast"; the broadcast is `ScalarOperand(expression.Evaluate(context))`, and `NameReference.Evaluate` of a name bound to `Ghost!$A$1:$A$3` is not an error.
...
- [ ] **15.** `INDEX(<computed array>, 0)` = 1 on Aspose (implicit intersection of the whole array) where MySheet answers `#REF!` — already pinned as a pre-existing divergence in `tests/Danfma.MySheet.Tests/Expressions/MiniCseConsumerTests.cs`. Fold this into item 13's whole-row/whole-column work: the same `row < 1 || column < 1` rejection owns both.
...
- **Is a `LET`-bound name array-eligible after item 8?** `NamedReferences.TryResolveReference` resolves a `NameReference` through the LET scope first, so `SUM(LET(r,A1:A3,(r<>0)*1))` should become eligible as a side effect. Not measured on either side — pin whatever it does, and do not design for it.
