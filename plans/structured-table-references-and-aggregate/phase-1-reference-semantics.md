...
- [x] **B1.** FIX C item 16: "extend the up-front validation at :35 to `if (ranges[a].Count != length || !SameShape(ranges[0], ranges[a]))`" with `SameShape` returning true whenever either side's `Rows == 0`. Presented as making SUMPRODUCT obey Excel's documented dimension rule.
      ...
      *Correction:* Give the snapshot branch its shape: add a `PositionalRange(IReadOnlyList<ComputedValue>
      ...
      exercised.
- [x] **B2.** Item 18: add `ROW(INDIRECT("A2"))=2` and `ROW(INDIRECT("A2:A3"))=2` to tests/Danfma.MySheet.Tests/Parsing/ReferenceFunctionTests.cs "using its existing `Grid`/`N`/`T` helpers (:8-25)", claiming "Every one of these values was measured against the patched engine".
...
- [x] **2.** In Danfma.MySheet/Expressions/Lookup/Row.cs, insert `[var only] => ReferencePosition.Row(only, context),` between the `[] when context.CellId is { } id` arm at :33 and `_ => ComputedValue.Error(Error.Value)` at :34. Keep all four existing syntactic arms (:15-16, :22-28) unchanged.
...
- [x] **3.** In Danfma.MySheet/Expressions/Lookup/LookupFunctions.cs, insert `[var only] => ReferencePosition.Column(only, context),` in `record Column` (:272) between the `[] when context.CellId is { } id` arm at :293-295 and `_ => ComputedValue.Error(Error.Value)` at :296.
...
- [x] **4.** Change `ArrayEvaluation.IsArrayEligible(Expression)` (ArrayEvaluation.cs:118) to `IsArrayEligible(Expression, EvaluationContext)` and `Probe(Expression)` (:123) to `Probe(Expression, EvaluationContext)`, threading the context through the five recursive Probe calls (binary Left/Right, If Arguments[0]/[1]/[2]) and updating the four call sites: NumericAggregation.cs:110, Index.cs:24, Index.cs:171, OrderStatistics.cs:508.
...
- [x] **7.** Add `private sealed class ColumnNumbersOperand : ArrayOperand` to ArrayEvaluation.cs above `BinaryOperand` (:330), mirroring `RowNumbersOperand` (:302-328) but returning `ComputedValue.Number(_leftColumn + index % _columns)`; then add four `Column {...}` arms to Probe (RangeReference / AnchoredRangeReference => (true,true), OpenRangeReference => (false,false), `[NameReference or Reference]` => ResolveRowRange) and the four mirror arms to TryBuildOperand, using `bounds.LeftColumn`.
      *Files:* `Danfma.MySheet/Expressions/ArrayEvaluation.cs`
...
- [x] **13.** Update the comments on the now-unreachable `ComputedValueKind.Reference` arms in Danfma.MySheet.Excel/ExcelExport.cs:279 and :292 and Danfma.MySheet.Excel/ExcelMerge.cs:575-579 to say a cell value can no longer BE a reference (the boundary intersects it) and that the arm survives only as a total-switch guard. Keep all three arms.
...
- [x] **15.** Add `public static PositionalRange OpenArrayOrRange(Expression argument, EvaluationContext context)` to PositionalRange: when `argument is not Reference && ArrayEvaluation.IsArrayEligible(argument, context) && ArrayEvaluation.TryEvaluateStream(argument, context, out var stream)` return the stream backing, else delegate to the existing `Open(argument, context)`. Document that it is SUMPRODUCT-only.
      ...
      *Why:* An opt-in factory rather than folding the gate into `Open` (:42-48), because Open is shared with
      ...
      property.
- [x] **16.** In `PositionalRange.Next()` (CriteriaScan.cs:99-108), insert the array branch between the list branch and the cursor branch: `if (_streamRows > 0) { var position = _index++; var column = position / _streamRows; var row = position % _streamRows; return _stream.ElementAt(row * _streamColumns + column); }`.
      *Files:* `Danfma.MySheet/Expressions/CriteriaScan.cs`
...
- [x] **17.** In Danfma.MySheet/Expressions/Mathematics/SumProducts.cs, switch both `PositionalRange.Open` calls (:26 and :33) to `PositionalRange.OpenArrayOrRange`, and extend the up-front validation at :35 to `if (ranges[a].Count != length || !SameShape(ranges[0], ranges[a]))`. Add `private static bool SameShape(in PositionalRange first, in PositionalRange other) => first.Rows == 0 || other.Rows == 0 || (first.Rows == other.Rows && first.Columns == other.Columns);` as a member of the SumProduct record (NOT in `file static class SumOfPairs`).
...
- [x] **26.** Commit FIX B as `feat(eval):` (a documented-behaviour change to the cell boundary), FIX A as `feat(eval):`, FIX C as `fix(eval):`; never hand-edit CHANGELOG.md — versionize generates it from the Conventional Commit subjects, and S7 fixes the release at 3.17.0.
      *Files:* `CHANGELOG.md`
...
- [x] **27.** Route the unresolvable-argument fallback of `Rows` (Rows.cs), `Columns` and `Areas` (both in
...
- [x] `cd /Volumes/Work/Develop/MySheet && dotnet run --project tests/Danfma.MySheet.Tests/Danfma.MySheet.Tests.csproj -c Release`
...
- [x] `cd /Volumes/Work/Develop/MySheet && dotnet run --project tests/Danfma.MySheet.Excel.Tests/Danfma.MySheet.Excel.Tests.csproj -c Release`
...
- [x] `cd /Volumes/Work/Develop/MySheet && dotnet run --project tests/Danfma.MySheet.Tests/Danfma.MySheet.Tests.csproj -c Release -- --treenode-filter "/*/*/CellBoundaryIntersectionTests/*"`
...
- [x] `cd /Volumes/Work/Develop/MySheet && dotnet run --project tests/Danfma.MySheet.Tests/Danfma.MySheet.Tests.csproj -c Release -- --treenode-filter "/*/*/MathAggregateTests/*"`
      ...
      or wrong.
- [x] `cd /Volumes/Work/Develop/MySheet && dotnet run --project tests/Danfma.MySheet.Tests/Danfma.MySheet.Tests.csproj -c Release -- --treenode-filter "/*/*/CellStoreTests/*" && dotnet run --project tests/Danfma.MySheet.Tests/Danfma.MySheet.Tests.csproj -c Release -- --treenode-filter "/*/*/WarmStartSaveLoadTests/*"`
...
- [x] `cd /Volumes/Work/Develop/MySheet && git diff --stat -- Danfma.MySheet Danfma.MySheet.Excel`
...
**Status: Complete** — branch `feat/reference-semantics`, commits `32a67f6..c04e78b` (16 commits), pending merge to `main`.
Gates verified by the controller at HEAD: csharpier clean, Release build 0 warnings, core **1280/1280**
(baseline 1203 → +77 cases), Excel **90/90** (88 → +2), both frozen wire goldens intact, no MemoryPack union
tag touched, no public API removed (`RangeReference.TryGetBounds` private→internal; `ReferencePosition.TryResolve`
internal, the family's single entry point). Executed as five tasks + a deferred-minor sweep + a final wave, each
task spec-reviewed and fix-rounded; a whole-branch review then found two cross-task findings the task reviews
could not (both fixed). Ledger with every ruling: `.superpowers/sdd/phase-1-reference-semantics/progress.md`
(untracked, on the controller's machine).

### What landed (behaviour changes — all stated in commit bodies)

1. `ROW`/`COLUMN` accept any reference-producing expression (name, INDEX/OFFSET/INDIRECT/CHOOSE, `:` ranges with
   reference-returning endpoints, open ranges at their DECLARED bound); an unresolvable argument reports its own
   error (`#NAME?`/`#REF!`); a non-reference is now EVALUATED (`ROW(1/0)` → `#DIV/0!`, a volatile draws).
2. **Scope addition:** `ROWS`/`COLUMNS`/`AREAS` propagate the argument's error instead of answering 1, and
   (final wave) re-check the RESOLVED target's sheet through the same `ReferencePosition.TryResolve`, so
   `ROWS(INDEX(Ghost!A1:A3,2,1))` = `#REF!` like `ROW`. A scalar still counts as a 1x1 array (`ROWS(5)` = 1).
3. `ROW`/`COLUMN` in ARRAY position over a name or reference yield the position array in the mini-CSE
   (`SUM(ROW(MyName))` = 6, `COUNT(ROW(MyName))` 0 → 3 — a silent wrong answer fixed); COLUMN gained an array
   operand at all. The axis mirror was collapsed: live symbols are `PositionNumbersOperand`,
   `ProbePosition`/`TryBuildPositionOperand`, `ResolvePositionRange`, `PositionArgumentShape` (the plan's
   `RowNumbersOperand`/`ColumnNumbersOperand`/`ResolveRowRange`/`RowArgumentShape` never existed on the branch).
   A reference-returning FUNCTION argument stays scalar in array position (M2, draw-count pinned).
4. **Cell boundary = Excel's implicit intersection (S4 range half).** `=A1:A3` bare in C2 → A2 (was `#VALUE!`);
   `=MyName` / `=INDIRECT("MyName")` / `=OFFSET(...)` / `CHOOSE` / `LET` / unary `+` → the intersected scalar (was a
   leaked `Reference` value reading as blank); open ranges intersect on DECLARED bounds; 2-D and no-intersection →
   `#VALUE!`; union → `#VALUE!`; self-intersection → `#REF!` (cycle guard); shared-formula slaves intersect at
   their own row (M1: the needed `CaptureValue` arm was `SharedFormulaSlave`). Consequences: warm-start now
   persists such cells; xlsx export/merge write the intersected value, no longer `t="e"` `#VALUE!`.
5. `SUMPRODUCT` reads computed arrays (`SUMPRODUCT((A1:A3<>0)*1)` = 2, `SUMPRODUCT(ROW(A1:B2),COLUMN(A1:B2))` = 9,
   `SUMPRODUCT(ROW(A1:A3))` 1 → 6) through an opt-in `PositionalRange.OpenArrayOrRange` (the *IFS family is
   unchanged), transposed on read (row-major stream vs column-major range); the dimension rule is
   orientation-aware and pivots on the first KNOWN shape (`SUMPRODUCT(A1:A3,A1:C1)` 25 → `#VALUE!`;
   `SUMPRODUCT(MyName,A1:A3,A1:C1)` 125 → `#VALUE!`). B1 applied: the RangeSnapshot backing carries its
   rectangle, pinned by a ≥256-cell two-cell test (mutation-verified: only the second cell catches it).
6. Hardening from the sweep: a malformed host-built `RangeReference` id at the cell boundary → `#VALUE!` instead
   of `FormatException` escaping `GetCellValue`.
7. Docs rewritten EN + pt-BR (computed-value, workbook-and-expressions incl. a new "Implicit intersection at the
   cell boundary" section, function-reference rows, custom-functions, excel-interop, serialization); every cited
   number pinned by a committed test; inferences (cross-sheet positional rule, 2-D `#VALUE!`) labelled as such.

### Acceptance against the S8 corpus idiom (fixture A1=5, A2=0, A3=9)

`ROW(INDEX(A1:A3,1,1))` = 1 ✅ pinned. `SUMPRODUCT((A1:A3<>0)*1)` = 2 ✅. `=A1:A3` in C2 = 0 ✅. The corpus
`SMALL((ROW(A1:A3)-ROW(INDEX(A1:A3,1,1))+1)/((A1:A3<>"")*(A1:A3<>0)),k)` → `#DIV/0!` for k=1,2 — **Excel-correct
for SMALL** (it propagates the error element); the vector underneath is `[1, #DIV/0!, 3]` (pinned via
`COUNT` = 2 and the error-free `SMALL(IF(...))` = 1 / 3). **Phase 2's `AGGREGATE(15,6,…)` is what turns these into
the corpus's 1 and 3** — the before/after is pinned in `MiniCseConsumerTests`.

### Rulings that override the plan text (see ledger for cost-if-wrong)

Item 27's "rather than 1" = unresolvable arguments only; AREAS never returned 1 (`#VALUE!` pre-existing); the
`[]`-arm ordering rationale in items 2-3 was false (list patterns are arity-disjoint; the real, compiler-enforced
constraint is "after the single-argument syntactic arms"); the axis mirror was collapsed (elegance rule over
"verbatim"); the anchored fast-path arms stay although correctness-redundant (they skip a resolution on the
shared-formula hot path); `SameShape(ranges[0], …)` was replaced by a first-known-shape pivot; the
`CellReference` dereference uses `cell.Evaluate` (equivalent on memo/cycle/taint and works for non-A1 ids).

### Still open (carry to the master plan's open decisions / later phases)

- `SUM(ROW(Ghost!A1:A3))` = 6 where the scalar `ROW(...)` = `#REF!` (Excel `#REF!` both): the mini-CSE
  LITERAL fast path skips the resolved-target guard. Pinned as a known gap (`…KeepsTheSyntacticGap`) and
  documented for users. Closing it costs a guard on the hot path — decide with Phase 3, which lands on the same arms.
- Guard-then-resolve double resolution (ROW/ROWS family) and probe-then-build double resolution (mini-CSE): a
  side-effecting `:` endpoint can make the two draws disagree (`SUM(ROW(INDEX(A1:A3,TICK(),1):A3))` = 5, Excel 6;
  value unpinned). Fix = a per-evaluation resolution memo (M2's prescription).
- `Index.TryResolveReference` probes `IsArrayEligible` with no build — one wasted resolution.
- SUMPRODUCT's dimension rule is count-only for shapeless arguments (name/open range/union) — documented deviation;
  closing it means giving `ArgumentFlattening`'s fallback a shape (touches the *IFS shared helper).
- Malformed host-built `RangeReference` ids still throw on the ROW/ROWS/SUMPRODUCT paths (only the cell boundary
  was hardened) — pre-existing, `TryGetBounds` is now available to fix them.
- Unmeasured Excel semantics (need the Excel/Aspose oracle): 2-D implicit intersection (both axes vs `#VALUE!`),
  cross-sheet positional intersection, union intersection, `=IF(TRUE,A1:A3,B1)` in a cell (Excel intersects;
  MySheet `#VALUE!` — pinned both paths, IFS/SWITCH same).
- The non-A1 cell-id (`GetCellValueOverflow`) boundary path is untested.
- `ReferencePosition` hosts position + the counting family's error recovery — reviewer judged: do not split
  (would create a cycle); a rename is the only honest change and not worth the churn now.
- Watch item: `ArrayEvaluation.cs` 646 → ~880 lines, `CriteriaScan.cs` ~376. Extract the `ArrayOperand` tree into
  its own file BEFORE Phase 2 (AGGREGATE shares the scan) and Phase 3 (table node lands on the `or Reference` arms).
- Phase 3 contract: the table node MUST derive from `Reference` and return a concrete `RangeReference` from
  `TryResolveReference`, or it silently misses the `[NameReference or Reference]` arms and `ROW(T[Col])` degenerates
  to a scalar.

### Verification Plan — stale expectations corrected

Excel suite is **90**, not "88 unchanged" (two anchored array-path tests). Core counts `[Arguments]` rows, so
"1203 + new `[Test]` methods" never worked: **1203 → 1280**. Footprint is ~1024 engine lines across 16 modified +
2 new files (`ImplicitIntersection.cs`, `ReferencePosition.cs`), not "~300 across 9 + 2" — the estimate predates
item 27 and the axis collapse. `/tmp/reference-semantics-validated.patch` is superseded (names dead symbols).
