...
- **Should unary `+` be transparent in an array position?** This plan keeps it opaque (the brief's instruction and the safe choice): `SUM(+A1:A3)` = 6 today and after, through the reference value path. The cost is that `SUM(-(+A1:A3))` stays `#VALUE!` where Excel gives -6 — measured both today and through the prototype. Making `Plus` recurse into its operand for array purposes is a two-line change, but it would move `SUM(+A1:A3)` from the RANGE path to the ARRAY path, and those two paths do not agree on blanks and text (measured: `SUM(B1:B3)` = 1 with `B2`="x" and `B3` blank; whether the array path folds the same needs its own pin). Decide with a fixture that contains a blank, a text cell and a number, and pin both paths before changing anything.
