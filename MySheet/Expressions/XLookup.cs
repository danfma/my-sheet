...
public sealed partial record XLookup(Expression[] Arguments) : Function
{
    ...
    public override object? Compute(EvaluationContext context)
    {
        ...
        if (Arguments.Length >= 5 &&
            ValueCoercion.TryToNumber(Arguments[4].Compute(context), out matchMode) is { } matchError)
        {
            return matchError;
        }
        ...
        if (Arguments.Length >= 6 &&
            ValueCoercion.TryToNumber(Arguments[5].Compute(context), out searchMode) is { } searchError)
        {
            return searchError;
        }
    ...
    private static int FindMatch(object? lookup, List<object?> array, int count, int matchMode, bool reverse)
    {
