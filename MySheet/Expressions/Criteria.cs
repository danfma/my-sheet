...
internal sealed class Criteria
{
    ...
    {
        if (s.StartsWith(">=", StringComparison.Ordinal)) return (Op.GreaterOrEqual, s[2..]);
        if (s.StartsWith("<=", StringComparison.Ordinal)) return (Op.LessOrEqual, s[2..]);
        if (s.StartsWith("<>", StringComparison.Ordinal)) return (Op.NotEqual, s[2..]);
        if (s.StartsWith(">", StringComparison.Ordinal)) return (Op.Greater, s[1..]);
        if (s.StartsWith("<", StringComparison.Ordinal)) return (Op.Less, s[1..]);
        if (s.StartsWith("=", StringComparison.Ordinal)) return (Op.Equal, s[1..]);
        return (Op.Equal, s);
    }
