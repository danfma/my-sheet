...
| `LET` | `LET(name1, value1, [name2, value2, …], calculation)` | Vincula nomes utilizáveis em `calculation` (ex.: `=LET(x, A1*2, x+x)`). Os nomes são locais à fórmula. Um intervalo vinculado a um nome permanece um intervalo, então `LET(hdr, Data!$1:$1, MATCH(x, hdr, 0))` e `LET(r, A1:C9, SUM(r))` funcionam; uma única célula é vinculada por valor. Um nome vinculado a um array computado (um produtor, um operador sobre um intervalo, uma chamada elevada) transmite-se inteiro para um consumidor — `SUM(LET(f,FILTER(A1:A3,A1:A3>0),f))` = 14 — e mostra o elemento superior esquerdo quando lido sozinho — `=LET(f,FILTER(A1:A3,A1:A3>0),f)` = 5. |
...
| `OFFSET` | `OFFSET(reference, rows, cols, [height], [width])` | Uma referência deslocada (e opcionalmente redimensionada) a partir de uma referência inicial; pode retornar uma referência multicélula para consumidores que aceitam intervalos. Uma base aberta começa na primeira célula declarada dela, não na caixa delimitadora populada, e um resultado multicélula eleva célula a célula sob um operador em contexto de array. |
