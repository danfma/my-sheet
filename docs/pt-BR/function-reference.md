...
| `HLOOKUP` | `HLOOKUP(lookup_value, table_range, row_index_num, [range_lookup])` | Pesquisa horizontal na primeira linha de uma tabela; exata ou aproximada; uma tabela escalar é tratada como 1x1 (uma falha → `#N/A`), `row_index_num` < 1 → `#VALUE!`, além da tabela → `#REF!`. |
...
| `VLOOKUP` | `VLOOKUP(lookup_value, table_range, col_index_num, [range_lookup])` | Pesquisa vertical na primeira coluna de uma tabela; exata ou aproximada; uma tabela escalar é tratada como 1x1 (uma falha → `#N/A`). |
| `XLOOKUP` | `XLOOKUP(lookup_value, lookup_range, return_range, [if_not_found], [match_mode], [search_mode])` | Pesquisa moderna, com contingência para "não encontrado" e modos de correspondência/busca. O intervalo de retorno precisa ter a mesma extensão no eixo de busca do intervalo pesquisado (linhas numa busca vertical, colunas numa horizontal), caso contrário → `#VALUE!`; células extras no outro eixo do intervalo de retorno são permitidas. |
