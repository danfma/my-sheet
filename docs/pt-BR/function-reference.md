...
| `COUNTIF` | `COUNTIF(range, criteria)` | Conta células correspondentes. Um `IF` condicionado por array pode selecionar um intervalo de referência; o MySheet usa o primeiro elemento da condição (sua convenção CSE), enquanto um ramo de array computado selecionado continua sendo `#REF!`. |
...
| `MATCH` | `MATCH(lookup_value, lookup_range, [match_type])` | Posição (base 1) de um valor em um intervalo (`match_type`: 1 aproximado crescente — padrão, 0 exato, -1 aproximado decrescente). Sobre um intervalo aberto, a posição é relativa à origem declarada dele, não à caixa delimitadora populada: `MATCH("x",$4:$4,0)` é `3` para C4. |
