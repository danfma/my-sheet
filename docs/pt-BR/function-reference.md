...
| `SUMIF` | `SUMIF(range, criteria, [sum_range])` | Soma células correspondentes. Um `sum_range` fornecido é redimensionado para a forma de `range` a partir do canto superior esquerdo (`SUMIF(A1:A3,">0",B1)` lê `B1:B3`); um seletor `IF` cujo ramo selecionado é uma referência é aceito no slot de intervalo. |
| `SUMIFS` | `SUMIFS(sum_range, criteria_range1, criteria1, …)` | Soma sob múltiplos pares de intervalo e critério; ao contrário de `SUMIF`, todos os intervalos já precisam ter o mesmo tamanho (`#VALUE!` caso contrário). |
...
| `AVERAGEIF` | `AVERAGEIF(range, criteria, [average_range])` | Média das células correspondentes; um `average_range` fornecido é redimensionado para a forma de `range` a partir do canto superior esquerdo; nenhuma correspondência numérica → `#DIV/0!`. |
