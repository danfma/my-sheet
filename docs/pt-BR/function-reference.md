...
| --- | --- | --- |
| `AND` | `AND(logical1, [logical2], …)` | `TRUE` se todo argumento for considerado verdadeiro; operandos de texto e em branco são ignorados (seja um literal ou alcançado por meio de uma referência); nenhum valor avaliável → `#VALUE!`. Um operando de texto é IGNORADO aqui, e **não** convertido — este é o único lugar em que o texto `"TRUE"`/`"FALSE"` NÃO se torna um booleano como acontece na condição do [`IF`](#lógicas-12), no `NOT` e no `IFS`: `AND("FALSE",TRUE)` é `TRUE` (a conversão daria `FALSE`), `OR("TRUE",FALSE)` é `FALSE`, `AND("yes",TRUE)` é `TRUE` em vez de erro, e `XOR("TRUE","FALSE")` é `#VALUE!` porque nada avaliável sobra. Medido no Aspose.Cells 26.6.0 (2026-09-10), com a entrada normal e a entrada em array concordando. |
| `FALSE` | `FALSE()` | O valor lógico `FALSE` (forma de função do literal). |
| `IF` | `IF(condition, value_if_true, [value_if_false])` | Condicional; apenas o ramo escolhido é avaliado. O compartimento `condition` também lê o TEXTO `"TRUE"` e `"FALSE"` como o booleano correspondente — exatamente essas duas palavras, comparadas SEM diferenciar maiúsculas de minúsculas e **sem** remoção de espaços, seja como literal ou dentro de uma célula: `IF("TRUE",1,0)` = 1, `IF("false",1,0)` = 0 e `IF(A1,1,0)` = 1 para uma célula com `"true"`. Qualquer outra coisa num compartimento booleano continua `#VALUE!` — `IF(" TRUE ",1,0)` (os espaços não são removidos), `IF("yes",1,0)` e o texto numérico `IF("1",1,0)` / `IF("0",1,0)` / `IF("",1,0)`. A regra pertence APENAS ao compartimento da condição: aritmética e comparação não convertem (`"TRUE"+0` e `--"TRUE"` são `#VALUE!`; `"TRUE"=TRUE` é `FALSE`), e **`AND`/`OR`/`XOR` IGNORAM um argumento de texto em vez de convertê-lo** — `AND("FALSE",TRUE)` é `TRUE`, não `FALSE`. A mesma regra alcança uma condição em ARRAY elemento por elemento, então `SUMPRODUCT(IF(C1:C3,1,0))` sobre `"TRUE"`/`"FALSE"`/`"true"` é 2. Todo valor deste parágrafo foi medido no Aspose.Cells 26.6.0 (2026-09-10), com a entrada normal e a entrada em array concordando em todos eles. |
...
| `IFS` | `IFS(test1, value1, [test2, value2], …)` | Primeiro valor cujo teste é `TRUE` (avaliação preguiçosa, como `IF`); nenhum teste `TRUE` → `#N/A`. Cada `test` é um compartimento booleano e segue a mesma regra estreita de texto da condição do [`IF`](#lógicas-12): `IFS("TRUE",1,TRUE,2)` = 1, `IFS("FALSE",1,TRUE,2)` = 2 (um FALSE convertido segue para o próximo teste em vez de gerar erro), e `IFS("yes",1,TRUE,2)` / `IFS(" TRUE ",1,TRUE,2)` / `IFS("1",1,TRUE,2)` são `#VALUE!`. Medido no Aspose.Cells 26.6.0 (2026-09-10), com a entrada normal e a entrada em array concordando. |
| `LET` | `LET(name1, value1, [name2, value2, …], calculation)` | Vincula nomes utilizáveis em `calculation` (ex.: `=LET(x, A1*2, x+x)`). Os nomes são locais à fórmula. Um intervalo vinculado a um nome permanece uma referência mesmo quando devolvido diretamente ou revinculado (`ROWS(LET(x,A1:A3,x))` = 3), então `LET(hdr, Data!$1:$1, MATCH(x, hdr, 0))` e `LET(r, A1:C9, SUM(r))` funcionam; uma única célula é vinculada por valor. Um nome vinculado a um array computado transmite-se inteiro para um consumidor e mostra o elemento superior esquerdo quando lido sozinho. |
| `NOT` | `NOT(logical)` | Negação lógica. Seu argumento é um compartimento booleano, então segue a mesma regra estreita de texto da condição do [`IF`](#lógicas-12): as duas palavras `TRUE` e `FALSE` como texto, sem diferenciar maiúsculas de minúsculas e sem remover espaços — `NOT("TRUE")` = `FALSE`, `NOT("FALSE")` = `TRUE`, `NOT("yes")` = `#VALUE!` — elemento por elemento sobre um array (`SUMPRODUCT(--NOT(A1:A2))` = 1 para células com `"TRUE"`/`"FALSE"`). Medido no Aspose.Cells 26.6.0 (2026-09-10), com a entrada normal e a entrada em array concordando. |
| `OR` | `OR(logical1, [logical2], …)` | `TRUE` se algum argumento for considerado verdadeiro; operandos de texto e em branco são ignorados (seja um literal ou alcançado por meio de uma referência); nenhum valor avaliável → `#VALUE!`. Um operando de texto é IGNORADO aqui, e **não** convertido — este é o único lugar em que o texto `"TRUE"`/`"FALSE"` NÃO se torna um booleano como acontece na condição do [`IF`](#lógicas-12), no `NOT` e no `IFS`: `AND("FALSE",TRUE)` é `TRUE` (a conversão daria `FALSE`), `OR("TRUE",FALSE)` é `FALSE`, `AND("yes",TRUE)` é `TRUE` em vez de erro, e `XOR("TRUE","FALSE")` é `#VALUE!` porque nada avaliável sobra. Medido no Aspose.Cells 26.6.0 (2026-09-10), com a entrada normal e a entrada em array concordando. |
| `SWITCH` | `SWITCH(expression, value1, result1, …, [default])` | Primeiro resultado cujo valor é igual a `expression` (avaliação preguiçosa; semântica de igualdade do `=`); sem correspondência → o padrão ou `#N/A`. `expression` NÃO é um compartimento booleano, então a regra de texto do [`IF`](#lógicas-12) não o alcança: `SWITCH("TRUE",TRUE,1,0)` e `SWITCH(TRUE,"TRUE",1,0)` caem no padrão, enquanto `SWITCH("TRUE","TRUE",1,0)` = 1 por igualdade de texto com texto. Medido no Aspose.Cells 26.6.0 (2026-09-10), com a entrada normal e a entrada em array concordando. |
...
| `SUMIF` | `SUMIF(range, criteria, [sum_range])` | Soma células correspondentes. A regra da família de critérios distingue um critério vindo de célula ausente (zero numérico) de texto vazio: `""` implícito corresponde a brancos e texto vazio, `"="` explícito apenas a células realmente em branco e `"<>"` apenas a células não vazias. Um `sum_range` fornecido é redimensionado para a forma de `range` a partir do canto superior esquerdo (`SUMIF(A1:A3,">0",B1)` lê `B1:B3`); um seletor `IF` cujo ramo selecionado é uma referência é aceito no slot de intervalo. |
| `SUMIFS` | `SUMIFS(sum_range, criteria_range1, criteria1, …)` | Soma sob múltiplos pares de intervalo e critério, com a mesma regra de critérios ausentes versus texto vazio do `COUNTIF`; ao contrário de `SUMIF`, todos os intervalos já precisam ter o mesmo tamanho (`#VALUE!` caso contrário). |
| `SUMPRODUCT` | `SUMPRODUCT(array1, [array2], …)` | Soma dos produtos posição a posição; entradas não numéricas contam como 0 — inclusive o `TRUE`/`FALSE` de uma comparação pura, e é por isso que o idioma do Excel multiplica por 1 — enquanto um elemento de erro se propaga. Aceita um [array computado](workbook-and-expressions.md#argumentos-implícitos-de-array) como argumento inteiro, misturado livremente com intervalos: `SUMPRODUCT((A1:A3<>0)*1)` = 2, `SUMPRODUCT(ROW(A1:B2)*COLUMN(A1:B2))` = 9 (o produto externo 2x2 de um 2x1 com um 1x2). Os argumentos precisam ter as mesmas **dimensões**, e não apenas a mesma contagem de células: `SUMPRODUCT(A1:A3,A1:C1)` (3x1 contra 1x3) → `#VALUE!`. O `SUMPRODUCT` também **não propaga** (*broadcast*) os próprios argumentos, ao contrário dos operadores: um 3x3 contra um 3x1 — ou até contra um intervalo 1x1 — é `#VALUE!` (`SUMPRODUCT(A1:C3,E1:E3)`, `SUMPRODUCT(A1:C3,E1:E1)` e `SUMPRODUCT(ROW(A1:B2),COLUMN(A1:B2))`, um 2x1 contra um 1x2); escreva a propagação *dentro* de um argumento, que é consumido como o array que ela computa (`SUMPRODUCT(A1:C3*E1:E3)` = 108, enquanto `SUMPRODUCT(A1:C3,E1:E3)` é `#VALUE!`) — o Excel concorda com tudo isso, medido no Aspose.Cells 26.6.0, em 2026-09-10, tanto digitado quanto inserido como array. Um argumento *sem formato* — um nome definido, um intervalo aberto, uma união — não carrega um retângulo que a engine consiga enxergar e é julgado apenas pela contagem de células, então `SUMPRODUCT(MyName,A1:C1)` sobre um nome 3x1 calcula onde o Excel responde `#VALUE!` (uma divergência documentada); todo argumento que *carrega* um retângulo continua sendo comparado com os demais, então `SUMPRODUCT(MyName,A1:A3,A1:C1)` é `#VALUE!`. |
...
| `AVERAGEIF` | `AVERAGEIF(range, criteria, [average_range])` | Média das células correspondentes, com a mesma regra de critérios ausentes versus texto vazio do `COUNTIF`; um `average_range` fornecido é redimensionado para a forma de `range` a partir do canto superior esquerdo; nenhuma correspondência numérica → `#DIV/0!`. |
| `AVERAGEIFS` | `AVERAGEIFS(average_range, criteria_range1, criteria1, …)` | Média sob múltiplos pares de intervalo e critério, com a mesma regra de critérios ausentes versus texto vazio do `COUNTIF`; nenhuma correspondência → `#DIV/0!`; incompatibilidade de formato → `#VALUE!`. |
...
| `COUNTIF` | `COUNTIF(range, criteria)` | Conta células correspondentes. Um critério vindo de célula ausente é o zero numérico, `""` corresponde a brancos e texto vazio, `"="` corresponde apenas a células realmente em branco e `"<>"` corresponde apenas a células não vazias. Um `IF` condicionado por array pode selecionar um intervalo de referência; o MySheet usa o primeiro elemento da condição (sua convenção CSE), enquanto um ramo de array computado selecionado continua sendo `#REF!`. |
| `COUNTIFS` | `COUNTIFS(criteria_range1, criteria1, …)` | Contagem sob múltiplos pares de intervalo e critério, com a mesma regra de critérios ausentes versus texto vazio do `COUNTIF`. |
...
| `HLOOKUP` | `HLOOKUP(lookup_value, table_range, row_index_num, [range_lookup])` | Pesquisa horizontal na primeira linha de uma tabela; exata ou aproximada. Uma chave de célula ausente segue a regra do zero numérico, enquanto uma chave de texto vazio corresponde a texto vazio armazenado ou produzido por fórmula, e não a uma célula realmente em branco; o modo exato retorna `#N/A` quando não existe candidato de texto, e o modo aproximado seleciona o último candidato de texto vazio. Uma tabela escalar ou de uma única célula resolvida é tratada como 1x1 e recebe a validação normal de modo/índice (uma falha → `#N/A`), `row_index_num` < 1 → `#VALUE!`, além da tabela → `#REF!`. |
| `INDEX` | `INDEX(reference, row_num, [column_num], [area_num])` | O valor em uma posição (base 1) dentro de uma referência. Para uma união, `area_num` seleciona sua área em base 1 antes da validação de linha/coluna; o padrão é 1 e ele é truncado (`INDEX((A1:A3,C1:C3),2,1,2)` seleciona `C2`). Um `area_num` zero ou negativo → `#VALUE!`; uma área além da união (ou a área 2 de uma referência única) → `#REF!`; um array que não seja referência com `area_num` → `#VALUE!`. Quando `row_num` ou `column_num` é `0`, a coluna/linha/área selecionada inteira é retornada como uma REFERÊNCIA, como o `OFFSET`: `SUM(INDEX(A1:C10,0,2))` soma a coluna B, `SUM(INDEX(A1:C10,3,0))` soma a linha 3, `INDEX(A1:C10,0,0)` é o intervalo inteiro, e a forma de 2 argumentos `INDEX(range,0)` sobre um intervalo de uma única coluna/linha é igualmente o intervalo inteiro. Um intervalo aberto conta suas posições a partir da origem declarada dele, não da caixa delimitadora populada: `INDEX($5:$10,0,5)` seleciona a coluna E absoluta. Um consumidor que aceita referências (`SUM`, `ROWS`/`COLUMNS`, `ROW`/`COLUMN`, o array de busca do `MATCH`, um `INDEX` aninhado, uma base de `OFFSET`, a extremidade de um intervalo `:`, …) lê isso como uma referência; sob um operador em contexto de array, um `INDEX` que retorna referência eleva célula a célula. Uma célula sozinha faz a interseção com ela como com qualquer outra referência. Aceita um [primeiro argumento implícito de array](workbook-and-expressions.md#argumentos-implícitos-de-array) (`INDEX(ROW(B2:B5),1)` = 2), incluindo a identidade `INDEX(ROW($A:$A), n)` que retorna `n` sem materializar a coluna — o `0` ali é um caso DIFERENTE, sem relação (veja o link acima). Um índice negativo → `#VALUE!`; um índice além da extensão → `#REF!`. |
| `INDIRECT` | `INDIRECT(ref_text, [a1])` | A referência nomeada por `ref_text`, resolvida em tempo de avaliação: o texto é interpretado como um corpo de fórmula no contexto da **planilha atual**, de modo que um `"A1"` sem qualificação significa o `A1` da planilha chamadora, enquanto `"Data!B2"` atravessa planilhas. Uma única célula é desreferenciada para o seu **valor** (`INDIRECT("A1")`); um resultado multicélula permanece uma **referência** para consumidores que entendem intervalos (`SUM(INDIRECT("A1:A3"))`, `ROWS(INDIRECT("A1:A3"))` = 3, e como extremidade de `:` — `SUM(INDIRECT("A1"):A3)`), de modo que, sozinha em uma célula, ela recebe [interseção implícita na fronteira da célula](workbook-and-expressions.md#interseção-implícita-na-fronteira-da-célula) (`=INDIRECT("A1:A3")` escrita em `B2` mostra `A2`; em `B5`, `#VALUE!`). [Nomes definidos](workbook-and-expressions.md#intervalos-nomeados) também são resolvidos, inclusive um montado em tempo de execução (`SUM(INDIRECT("R"&"ng"))`, `INDIRECT("Data!A"&2)`), e o mesmo vale para uma **referência estruturada**, escrita ou montada: `SUM(INDIRECT("Tabela1[Valor]"))` = `60` e `SUM(INDIRECT("Tabela1"))` = `66` sobre o fixture de tabela (pinado; veja [Tabelas](workbook-and-expressions.md#tabelas)). **Volátil**: a referência só é conhecida em tempo de avaliação, então a célula é sempre recalculada — veja [Funções voláteis](workbook-and-expressions.md#funções-voláteis). Tudo o que falha vira `#REF!`, nunca `#NAME?`: `a1` = `FALSE`/`0` (o estilo R1C1 não é suportado — apenas A1) ou um `a1` que não seja um número; um `ref_text` que não seja texto (um número, um lógico, uma célula numérica); um texto que não seja interpretável; e um nome ou uma planilha desconhecidos. |
| `LOOKUP` | `LOOKUP(lookup_value, lookup_vector, [result_vector])` | Forma vetorial (sempre aproximada: o maior valor ≤ pesquisado). Uma chave de célula ausente segue a regra do zero numérico, enquanto uma chave de texto vazio corresponde apenas a texto vazio armazenado ou produzido por fórmula, seleciona o último candidato desse tipo e retorna `#N/A` quando nenhum existe. Os vetores de busca e de resultado aceitam referências, constantes de array e vetores computados. A forma matricial de 2 argumentos busca na primeira linha e retorna da última linha quando o intervalo é mais largo que alto; caso contrário, primeira/última coluna. |
| `MATCH` | `MATCH(lookup_value, lookup_range, [match_type])` | Posição (base 1) de um valor em um intervalo (`match_type`: 1 aproximado crescente — padrão, 0 exato, -1 aproximado decrescente). Uma chave de célula ausente é o zero numérico; uma chave de texto vazio no modo exato ou aproximado corresponde a texto vazio armazenado ou produzido por fórmula, e não a uma célula realmente em branco, ou retorna `#N/A` quando não existe candidato de texto. Sobre um intervalo aberto, a posição é relativa à origem declarada dele, não à caixa delimitadora populada: `MATCH("x",$4:$4,0)` é `3` para C4. |
| `OFFSET` | `OFFSET(reference, rows, cols, [height], [width])` | Uma referência deslocada (e opcionalmente redimensionada) a partir de uma referência inicial; pode retornar uma referência multicélula para consumidores que aceitam intervalos. `height` ou `width` omitido herda essa dimensão da base; uma dimensão explícita positiva a substitui, zero é `#REF!`, e uma dimensão negativa se estende para cima ou para a esquerda a partir da origem deslocada usando o tamanho absoluto, portanto todo consumidor vê uma única janela coerente. Uma base aberta começa na primeira célula declarada dela, não na caixa delimitadora populada, e um resultado multicélula eleva célula a célula sob um operador em contexto de array. |
| `ROW` | `ROW([reference])` | Número da linha da referência (a linha superior para um intervalo) — ou da célula atual, quando chamada sem argumento. Aceita QUALQUER expressão que produza uma referência, não apenas uma referência literal: um nome definido, uma referência estruturada (`ROW(Tabela1[Valor])` = `2`, a primeira linha do corpo de dados — medido no Aspose.Cells 26.6.0, em 2026-09-11, nos dois modos de entrada; veja [Tabelas](workbook-and-expressions.md#tabelas)), `INDEX`/`OFFSET`/`INDIRECT`/`CHOOSE`, um intervalo `:` com extremidades que retornam referências (`ROW(INDEX(A1:A3,2,1))` = 2). Uma referência de coluna/linha inteira usa o limite DECLARADO (`ROW(A:A)` = 1, `ROW(A2:A)` = 2) — enquanto o `ROWS` da linha ao lado usa a extensão POPULADA em um eixo aberto; um argumento que não é referência (ou uma união) → `#VALUE!`, e um argumento que não consegue ser resolvido informa o próprio erro (`#NAME?`, `#REF!`). Em [posição de array](workbook-and-expressions.md#argumentos-implícitos-de-array) produz o vetor inteiro de números de linha — um número por linha, uma coluna Nx1, e não um por célula —, sobre um intervalo literal *e* sobre um nome ou um intervalo `:` que denote um (`SUM(ROW(A1:C3))` = 6 e `COUNT(ROW(A1:C3))` = 3 para a coluna 3x1 `[1,2,3]`, `SUM(ROW(MyName))` = 6 para um nome sobre três linhas). Ali um intervalo aberto é recusado. Uma função que retorna referência normalmente permanece escalar, exceto `INDEX`: `ROW(INDEX(A1:C3,0,1))` resolve essa referência uma vez e produz `[1,2,3]` em posição de array. Essas são as respostas com entrada como array, que é o modo que o MySheet implementa em todo lugar; digitado no Excel, o `ROW` de um retângulo é o único número da linha superior, então `SUM(ROW(A1:C3))` é 1 ali (medido no Aspose.Cells 26.6.0, em 2026-09-10). |
| `ROWS` | `ROWS(range)` | Número de linhas do intervalo. Sobre uma [referência de coluna/linha inteira](workbook-and-expressions.md#referências-de-coluna-e-linha-inteira), um eixo de linha aberto usa a extensão populada (`ROWS(A:A)` = linha populada máxima − linha populada mínima + 1, 0 se vazia — uma divergência documentada em relação à grade fixa do Excel), um limitado é exato (`ROWS(1:5)` = 5). Uma [referência estruturada](workbook-and-expressions.md#tabelas) responde a faixa que ela resolve — `ROWS(Tabela1)` = `3`, o corpo de dados, com o nome nu resolvendo pelo caminho de nomes como um nome definido (pinado). Um argumento que não consegue ser resolvido para uma referência informa o próprio erro (`ROWS(NoSuchName)` → `#NAME?`, `ROWS(INDIRECT("zz"))` → `#REF!`); um valor escalar simples conta como 1 (um array 1x1). Um **array computado** informa a extensão real dele, pelo mesmo portão que os consumidores de dobra usam: `ROWS(A1:C3*2)` = 3, `ROWS(LEN(A1:A3))` = 3, `ROWS(IF(A1:A3>0,A1:A3))` = 3, `ROWS(ROW(A1:A3))` = 3 e, sobre um [produtor de array dinâmico](workbook-and-expressions.md#produtores-de-array-dinâmico), `ROWS(FILTER(A1:A3,A1:A3>0))` = 2 e `ROWS(SEQUENCE(5))` = 5 — igual ao Excel, tanto digitado quanto inserido como array (medido no Aspose.Cells 26.6.0, 2026-09-10). O erro 1x1 próprio de um produtor é informado como ele mesmo, e não como referência ausente (`ROWS(FILTER(A1:A3,A1:A3>100))` = `#CALC!`, `ROWS(SEQUENCE(-1))` = `#VALUE!`), enquanto um ELEMENTO de erro dentro de um retângulo não esconde a forma (`ROWS(SORT(E1:E3))` = 3). |
...
| `UNIQUE` | `UNIQUE(array, [by_col], [exactly_once])` | As linhas distintas de `array` (ou, com `by_col`, as colunas distintas dele) na ordem da PRIMEIRA aparição, e um [produtor de array dinâmico](workbook-and-expressions.md#produtores-de-array-dinâmico): sozinho em uma célula mostra o elemento superior esquerdo, enquanto um consumidor lê o array inteiro (sobre 9, 5, 9, 0: `SUM` 14, `COUNTA` 3, `ROWS` 3). Dois candidatos casam quando cada par de células tem o mesmo TIPO e o mesmo valor: números numericamente, lógicos por valor, texto ORDINALMENTE — **DIFERENCIANDO maiúsculas de minúsculas**, então "a", "A", "b" mantém as três, enquanto `COUNTIF` e `MATCH` ao lado continuam sem diferenciar (medido; a página da Microsoft não diz nada sobre caixa, em nenhum sentido). Tipos nunca se cruzam (`1`, `"1"`, `TRUE` são três linhas), um BRANCO é chave própria — igual a nem 0, nem `""`, nem FALSE — e um ERRO é chave como qualquer outra, igual ao mesmo código de erro. `exactly_once` mantém os candidatos que ocorrem UMA vez, seguindo a página da Microsoft em vez do oráculo medido, que preserva a forma da contagem de distintos e **sobrescreve a frente dela** com os valores que ocorrem uma única vez, deixando cada elemento distinto que a lista não alcança onde estava — sobre 1, 1, 2, 3, 3, 4 isso responde 2, 4, 3, 4, um resultado de quatro linhas que carrega um valor que ocorre duas vezes (3) e uma duplicata de um que não ocorre (4), contradito pela própria contagem de linhas dele. Nada restante é `#CALC!`. Um `array` de intervalo aberto é recusado com `#VALUE!`. |
| `VLOOKUP` | `VLOOKUP(lookup_value, table_range, col_index_num, [range_lookup])` | Pesquisa vertical na primeira coluna de uma tabela; exata ou aproximada. Uma chave de célula ausente segue a regra do zero numérico, enquanto uma chave de texto vazio corresponde a texto vazio armazenado ou produzido por fórmula, e não a uma célula realmente em branco; o modo exato retorna `#N/A` quando não existe candidato de texto, e o modo aproximado seleciona o último candidato de texto vazio. Uma tabela escalar ou de uma única célula resolvida é tratada como 1x1 e recebe a validação normal de modo/índice (uma falha → `#N/A`). Uma tabela que é uma célula direta com erro retorna `#N/A` no modo exato antes dos limites do índice, enquanto o modo aproximado propaga o erro dela; uma célula de erro nomeada propaga seu erro nos dois modos. |
| `XLOOKUP` | `XLOOKUP(lookup_value, lookup_range, return_range, [if_not_found], [match_mode], [search_mode])` | Pesquisa moderna, com contingência para "não encontrado" e modos de correspondência/busca. Uma chave de célula ausente corresponde a uma célula de busca realmente em branco, enquanto uma chave de texto vazio corresponde a texto vazio armazenado ou produzido por fórmula, e não a uma célula realmente em branco. Nos modos exato e aproximado, retorna o resultado de "não encontrado" quando não existe candidato de texto; com vários candidatos, os modos de busca para frente selecionam o primeiro e os modos reversos selecionam o último. O array de busca precisa ser unidimensional. O array de retorno precisa ter a mesma extensão no eixo de busca (linhas numa busca vertical, colunas numa horizontal), caso contrário → `#VALUE!`; células extras no outro eixo do array de retorno são permitidas, e a linha ou coluna correspondente permanece disponível para consumidores de intervalo. Uma busca 1x1 é horizontal, então seu array de retorno precisa ter exatamente uma coluna. Referências, arrays vinculados por LET e arrays computados de nomes definidos seguem a mesma regra, e cada operando é avaliado uma vez. |
| `XMATCH` | `XMATCH(lookup_value, lookup_range, [match_mode], [search_mode])` | Posição (base 1) com os modos do `XLOOKUP` (0 exato — padrão, -1 exato-ou-menor, 1 exato-ou-maior, 2 curinga). Uma chave de célula ausente corresponde a uma célula de busca realmente em branco, enquanto uma chave de texto vazio corresponde a texto vazio armazenado ou produzido por fórmula, e não a uma célula realmente em branco. Nos modos exato e aproximado, retorna `#N/A` quando não existe candidato de texto; com vários candidatos, os modos de busca para frente selecionam o primeiro e os modos reversos selecionam o último. Os modos de busca 1/-1 e 2/-2 são aceitos; todos fazem varredura linear, e 2/-2 concordam com o Excel somente sobre dados ordenados. Sobre um intervalo aberto, a posição é relativa à origem declarada dele, como no `MATCH`. |

## Informações (18)

As funções `IS*` inspecionam o valor avaliado sem coerção (`ISNUMBER("19")` é `FALSE`) e nunca propagam
erros — elas os relatam. Consumidores escalares de informação/erro (`ISBLANK`, `ISERR`, `ISERROR`,
`ISLOGICAL`, `ISNA`, `ISNONTEXT`, `ISNUMBER`, `ISTEXT`, `N`, `IFERROR` e `IFNA`) aplicam primeiro a
interseção implícita pela posição de linha/coluna a uma referência multicélula, seja ela um intervalo
literal, coluna de tabela, nome definido, intervalo aberto ou referência entre planilhas.
...
| `ISREF` | `ISREF(value)` | `TRUE` quando o argumento é uma referência (célula/intervalo/união) — uma verificação sintática, independentemente do valor. Uma [referência estruturada](workbook-and-expressions.md#tabelas) responde `TRUE` quando resolve (`ISREF(Tabela1[Valor])`, e o `ISREF(Tabela1)` nu pelo caminho de nomes) e `FALSE` quando não resolve (`ISREF(Tabela1[#Totals])` sobre uma tabela sem linha de totais) — ambas pinadas. |
