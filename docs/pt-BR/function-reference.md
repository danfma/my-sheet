...
| `XMATCH` | `XMATCH(lookup_value, lookup_range, [match_mode], [search_mode])` | Posição (base 1) com os modos do `XLOOKUP` (0 exato — padrão, -1 exato-ou-menor, 1 exato-ou-maior, 2 curinga; busca 1/-1 e binária 2/-2). Sobre um intervalo aberto, a posição é relativa à origem declarada dele, como no `MATCH`. |
