...
| `COUNTIF` | `COUNTIF(range, criteria)` | Counts matching cells. An array-conditioned `IF` may select a reference range; MySheet uses the first condition element (its CSE convention), while a selected computed-array branch remains `#REF!`. |
...
| `MATCH` | `MATCH(lookup_value, lookup_range, [match_type])` | 1-based position of a value in a range (`match_type`: 1 approximate ascending — default, 0 exact, -1 approximate descending). Over an open range its position is relative to that range's declared origin, not its populated bounding box: `MATCH("x",$4:$4,0)` is `3` for C4. |
