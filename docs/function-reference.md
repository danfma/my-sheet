...
| `LET` | `LET(name1, value1, [name2, value2, …], calculation)` | Binds names usable in `calculation` (e.g. `=LET(x, A1*2, x+x)`). Names are local to the formula. A range bound to a name stays a range, so `LET(hdr, Data!$1:$1, MATCH(x, hdr, 0))` and `LET(r, A1:C9, SUM(r))` work; a single cell is bound by value. A name bound to a computed array (a producer, an operator over a range, a lifted call) streams whole to a consumer — `SUM(LET(f,FILTER(A1:A3,A1:A3>0),f))` = 14 — and shows its top-left when read bare — `=LET(f,FILTER(A1:A3,A1:A3>0),f)` = 5. |
