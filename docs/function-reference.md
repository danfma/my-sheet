...
| --- | --- | --- |
| `AND` | `AND(logical1, [logical2], …)` | `TRUE` if every argument is truthy; text and blank operands are ignored (whether a literal or reached through a reference); no evaluable value → `#VALUE!`. A text operand is IGNORED here, **not** coerced — this is the one place where the text `"TRUE"`/`"FALSE"` does NOT become a boolean the way it does in [`IF`](#logical-12)'s condition, `NOT` and `IFS`: `AND("FALSE",TRUE)` is `TRUE` (coercion would give `FALSE`), `OR("TRUE",FALSE)` is `FALSE`, `AND("yes",TRUE)` is `TRUE` rather than an error, and `XOR("TRUE","FALSE")` is `#VALUE!` because nothing evaluable survives. Measured on Aspose.Cells 26.6.0 (2026-09-10), plain and array-entered entry agreeing. |
| `FALSE` | `FALSE()` | The logical value `FALSE` (function form of the literal). |
| `IF` | `IF(condition, value_if_true, [value_if_false])` | Conditional; only the taken branch is evaluated. The `condition` slot also reads the TEXT `"TRUE"` and `"FALSE"` as the matching boolean — exactly those two words, compared case-INSENSITIVELY and **not** trimmed, whether written as a literal or held in a cell: `IF("TRUE",1,0)` = 1, `IF("false",1,0)` = 0, and `IF(A1,1,0)` = 1 for a cell holding `"true"`. Anything else in a boolean slot is still `#VALUE!` — `IF(" TRUE ",1,0)` (the padding is not stripped), `IF("yes",1,0)`, and the numeric text `IF("1",1,0)` / `IF("0",1,0)` / `IF("",1,0)`. The rule belongs to the condition slot ALONE: arithmetic and comparison do not coerce (`"TRUE"+0` and `--"TRUE"` are `#VALUE!`; `"TRUE"=TRUE` is `FALSE`), and **`AND`/`OR`/`XOR` IGNORE a text argument instead of coercing it** — `AND("FALSE",TRUE)` is `TRUE`, not `FALSE`. The same rule reaches an ARRAY condition element by element, so `SUMPRODUCT(IF(C1:C3,1,0))` over `"TRUE"`/`"FALSE"`/`"true"` is 2. Every value in this paragraph is measured on Aspose.Cells 26.6.0 (2026-09-10), plain and array-entered entry agreeing on all of them. |
...
| `IFS` | `IFS(test1, value1, [test2, value2], …)` | First value whose test is `TRUE` (lazy, like `IF`); no `TRUE` test → `#N/A`. Each `test` is a boolean slot and takes the same narrow text rule as [`IF`](#logical-12)'s condition: `IFS("TRUE",1,TRUE,2)` = 1, `IFS("FALSE",1,TRUE,2)` = 2 (a coerced FALSE falls through rather than erroring), and `IFS("yes",1,TRUE,2)` / `IFS(" TRUE ",1,TRUE,2)` / `IFS("1",1,TRUE,2)` are `#VALUE!`. Measured on Aspose.Cells 26.6.0 (2026-09-10), plain and array-entered entry agreeing. |
| `LET` | `LET(name1, value1, [name2, value2, …], calculation)` | Binds names usable in `calculation` (e.g. `=LET(x, A1*2, x+x)`). Names are local to the formula. A range bound to a name stays a reference even when returned directly or rebound (`ROWS(LET(x,A1:A3,x))` = 3), so `LET(hdr, Data!$1:$1, MATCH(x, hdr, 0))` and `LET(r, A1:C9, SUM(r))` work; a single cell is bound by value. A name bound to a computed array streams whole to a consumer and shows its top-left when read bare. |
| `NOT` | `NOT(logical)` | Logical negation. Its argument is a boolean slot, so it takes the same narrow text rule as [`IF`](#logical-12)'s condition: the two words `TRUE` and `FALSE` as text, case-insensitive and not trimmed — `NOT("TRUE")` = `FALSE`, `NOT("FALSE")` = `TRUE`, `NOT("yes")` = `#VALUE!` — element by element over an array (`SUMPRODUCT(--NOT(A1:A2))` = 1 for cells holding `"TRUE"`/`"FALSE"`). Measured on Aspose.Cells 26.6.0 (2026-09-10), plain and array-entered entry agreeing. |
| `OR` | `OR(logical1, [logical2], …)` | `TRUE` if any argument is truthy; text and blank operands are ignored (whether a literal or reached through a reference); no evaluable value → `#VALUE!`. A text operand is IGNORED here, **not** coerced — this is the one place where the text `"TRUE"`/`"FALSE"` does NOT become a boolean the way it does in [`IF`](#logical-12)'s condition, `NOT` and `IFS`: `AND("FALSE",TRUE)` is `TRUE` (coercion would give `FALSE`), `OR("TRUE",FALSE)` is `FALSE`, `AND("yes",TRUE)` is `TRUE` rather than an error, and `XOR("TRUE","FALSE")` is `#VALUE!` because nothing evaluable survives. Measured on Aspose.Cells 26.6.0 (2026-09-10), plain and array-entered entry agreeing. |
| `SWITCH` | `SWITCH(expression, value1, result1, …, [default])` | First result whose value equals `expression` (lazy; `=` equality semantics); no match → default or `#N/A`. `expression` is NOT a boolean slot, so [`IF`](#logical-12)'s text rule does not reach it: `SWITCH("TRUE",TRUE,1,0)` and `SWITCH(TRUE,"TRUE",1,0)` both take the default, while `SWITCH("TRUE","TRUE",1,0)` = 1 on text-to-text equality. Measured on Aspose.Cells 26.6.0 (2026-09-10), plain and array-entered entry agreeing. |
...
| `SUMIF` | `SUMIF(range, criteria, [sum_range])` | Sums matching cells. The criteria-family rule distinguishes an absent-cell criterion (numeric zero) from empty text: implicit `""` matches blanks and empty text, explicit `"="` only truly blank cells, and `"<>"` only nonblank cells. A supplied `sum_range` is resized to `range`'s shape from its top-left (`SUMIF(A1:A3,">0",B1)` reads `B1:B3`); an `IF` selector whose selected branch is a reference is accepted in the range slot. |
| `SUMIFS` | `SUMIFS(sum_range, criteria_range1, criteria1, …)` | Sum under multiple criteria-range pairs, with the same absent-versus-empty-text criteria rule as `COUNTIF`; unlike `SUMIF`, every range must already have the same size (`#VALUE!` otherwise). |
| `SUMPRODUCT` | `SUMPRODUCT(array1, [array2], …)` | Sum of the position-wise products; non-numeric entries count as 0 — including the `TRUE`/`FALSE` of a bare comparison, which is why the Excel idiom multiplies by 1 — while an error element propagates. Accepts a [computed array](workbook-and-expressions.md#implicit-array-arguments) as a whole argument, mixed freely with ranges: `SUMPRODUCT((A1:A3<>0)*1)` = 2, `SUMPRODUCT(ROW(A1:B2)*COLUMN(A1:B2))` = 9 (the 2x2 outer product of a 2x1 and a 1x2). The arguments must share the same **dimensions**, not merely the same cell count: `SUMPRODUCT(A1:A3,A1:C1)` (3x1 against 1x3) → `#VALUE!`. `SUMPRODUCT` also **does not broadcast** its own arguments, where the operators do: a 3x3 against a 3x1 — or even against a 1x1 range — is `#VALUE!` (`SUMPRODUCT(A1:C3,E1:E3)`, `SUMPRODUCT(A1:C3,E1:E1)`, and `SUMPRODUCT(ROW(A1:B2),COLUMN(A1:B2))` for a 2x1 against a 1x2); write the broadcast *inside* one argument instead, which is consumed as the array it computes (`SUMPRODUCT(A1:C3*E1:E3)` = 108 where `SUMPRODUCT(A1:C3,E1:E3)` is `#VALUE!`) — Excel agrees on all of it, measured on Aspose.Cells 26.6.0, 2026-09-10, typed and array-entered alike. A *shapeless* argument — a defined name, an open range, a union — carries no rectangle the engine can see and is judged by cell count alone, so `SUMPRODUCT(MyName,A1:C1)` over a 3x1 name computes where Excel answers `#VALUE!` (a documented deviation); every argument that does carry a rectangle is still compared with the others, so `SUMPRODUCT(MyName,A1:A3,A1:C1)` is `#VALUE!`. |
...
| `AVERAGEIF` | `AVERAGEIF(range, criteria, [average_range])` | Average of matching cells, with the same absent-versus-empty-text criteria rule as `COUNTIF`; a supplied `average_range` is resized to `range`'s shape from its top-left; no numeric match → `#DIV/0!`. |
| `AVERAGEIFS` | `AVERAGEIFS(average_range, criteria_range1, criteria1, …)` | Average under multiple criteria-range pairs, with the same absent-versus-empty-text criteria rule as `COUNTIF`; no match → `#DIV/0!`; shape mismatch → `#VALUE!`. |
...
| `COUNTIF` | `COUNTIF(range, criteria)` | Counts matching cells. An absent-cell criterion is numeric zero, `""` matches blanks and empty text, `"="` matches only truly blank cells, and `"<>"` matches only nonblank cells. An array-conditioned `IF` may select a reference range; MySheet uses the first condition element (its CSE convention), while a selected computed-array branch remains `#REF!`. |
| `COUNTIFS` | `COUNTIFS(criteria_range1, criteria1, …)` | Count under multiple criteria-range pairs, with the same absent-versus-empty-text criteria rule as `COUNTIF`. |
...
| `HLOOKUP` | `HLOOKUP(lookup_value, table_range, row_index_num, [range_lookup])` | Horizontal lookup in the first row of a table; exact or approximate. An absent-cell key follows the numeric-zero rule, while an empty-text key matches stored or formula-produced empty text rather than a truly blank cell; exact mode returns `#N/A` when no text candidate exists, and approximate mode selects the last exact-text candidate. A scalar or resolved single-cell table is treated as 1x1 and receives the normal mode/index validation (a miss → `#N/A`), `row_index_num` < 1 → `#VALUE!`, beyond the table → `#REF!`. |
| `INDEX` | `INDEX(reference, row_num, [column_num], [area_num])` | The value at a 1-based position inside a reference. For a union, `area_num` selects its 1-based area before row/column validation; it defaults to 1 and is truncated (`INDEX((A1:A3,C1:C3),2,1,2)` selects `C2`). Zero or negative `area_num` → `#VALUE!`; an area past the union (or area 2 of a single reference) → `#REF!`; a non-reference array with `area_num` → `#VALUE!`. When `row_num` or `column_num` is `0`, the whole column/row/selected area is returned as a REFERENCE, like `OFFSET`: `SUM(INDEX(A1:C10,0,2))` sums column B, `SUM(INDEX(A1:C10,3,0))` sums row 3, `INDEX(A1:C10,0,0)` is the whole range, and the 2-arg `INDEX(range,0)` over a one-column/one-row range is likewise the whole range. An open range counts its positions from its declared origin, not its populated bounding box: `INDEX($5:$10,0,5)` selects absolute column E. A range-aware consumer (`SUM`, `ROWS`/`COLUMNS`, `ROW`/`COLUMN`, `MATCH`'s lookup array, a nested `INDEX`, an `OFFSET` base, a `:` range endpoint, …) reads it as a reference; under an operator in an array context, a reference-returning `INDEX` lifts cell by cell. A bare cell intersects it like any other reference. Accepts an [implicit array first argument](workbook-and-expressions.md#implicit-array-arguments) (`INDEX(ROW(B2:B5),1)` = 2), including the `INDEX(ROW($A:$A), n)` identity that returns `n` without materializing the column — `0` there is a DIFFERENT, unrelated case (see the link above). A negative index → `#VALUE!`; an index past the extent → `#REF!`. |
| `INDIRECT` | `INDIRECT(ref_text, [a1])` | The reference named by `ref_text`, resolved at evaluation time: the text is parsed as a formula body in the **current sheet's** context, so an unqualified `"A1"` means the calling sheet's `A1` while `"Data!B2"` crosses sheets. A single cell is dereferenced to its **value** (`INDIRECT("A1")`); a multi-cell result stays a **reference** for range-aware consumers (`SUM(INDIRECT("A1:A3"))`, `ROWS(INDIRECT("A1:A3"))` = 3, and as a `:` endpoint — `SUM(INDIRECT("A1"):A3)`), so bare in a cell it gets [implicit intersection at the cell boundary](workbook-and-expressions.md#implicit-intersection-at-the-cell-boundary) (`=INDIRECT("A1:A3")` written in `B2` shows `A2`, in `B5` `#VALUE!`). [Defined names](workbook-and-expressions.md#named-ranges) resolve too, including one assembled at run time (`SUM(INDIRECT("R"&"ng"))`, `INDIRECT("Data!A"&2)`), and so does a **structured reference**, spelled or assembled: `SUM(INDIRECT("Tabela1[Valor]"))` = `60` and `SUM(INDIRECT("Tabela1"))` = `66` over the table fixture (pinned; see [Tables](workbook-and-expressions.md#tables)). **Volatile**: the reference is known only at evaluation time, so the cell is always recomputed — see [Volatile functions](workbook-and-expressions.md#volatile-functions). Everything that fails is `#REF!`, never `#NAME?`: `a1` = `FALSE`/`0` (R1C1 style is not supported — A1 only) or an `a1` that is not a number; a `ref_text` that is not text (a number, a logical, a numeric cell); text that does not parse; and an unknown name or sheet. |
| `LOOKUP` | `LOOKUP(lookup_value, lookup_vector, [result_vector])` | Vector form (always approximate: largest value ≤ lookup). An absent-cell key follows the numeric-zero rule, while an empty-text key matches only stored or formula-produced empty text, selects the last such candidate, and returns `#N/A` when none exists. Both lookup and result vectors accept references, array constants and computed vectors. The 2-argument array form searches the first row and returns from the last row when the range is wider than tall, otherwise first/last column. |
| `MATCH` | `MATCH(lookup_value, lookup_range, [match_type])` | 1-based position of a value in a range (`match_type`: 1 approximate ascending — default, 0 exact, -1 approximate descending). An absent-cell key is numeric zero; an empty-text key in exact or approximate mode matches stored or formula-produced empty text rather than a truly blank cell, or returns `#N/A` when no text candidate exists. Over an open range its position is relative to that range's declared origin, not its populated bounding box: `MATCH("x",$4:$4,0)` is `3` for C4. |
| `OFFSET` | `OFFSET(reference, rows, cols, [height], [width])` | A reference displaced (and optionally resized) from a starting reference; may return a multi-cell reference for range-aware consumers. An omitted `height` or `width` inherits that dimension from the base; an explicit positive dimension replaces it, zero is `#REF!`, and a negative dimension extends upward or leftward from the displaced origin using its absolute size, so every consumer sees one coherent window. An open base starts at its declared first cell, not its populated bounding box, and a multi-cell result lifts cell by cell under an operator in an array context. |
| `ROW` | `ROW([reference])` | Row number of the reference (top row for a range) — or of the current cell when called with no argument. Accepts ANY reference-producing expression, not only a literal reference: a defined name, a structured reference (`ROW(Tabela1[Valor])` = `2`, the data body's first row — measured on Aspose.Cells 26.6.0, 2026-09-11, both entry modes; see [Tables](workbook-and-expressions.md#tables)), `INDEX`/`OFFSET`/`INDIRECT`/`CHOOSE`, a `:` range with reference-returning endpoints (`ROW(INDEX(A1:A3,2,1))` = 2). A whole-column/row reference uses its DECLARED bound (`ROW(A:A)` = 1, `ROW(A2:A)` = 2) — where the adjacent `ROWS` uses the POPULATED extent on an open axis; a non-reference (or a union) → `#VALUE!`, and an argument that fails to resolve reports its own error (`#NAME?`, `#REF!`). In an [array position](workbook-and-expressions.md#implicit-array-arguments) it yields the whole row-number vector — one number per row, an Nx1 column, not one per cell — over a literal range *and* over a name or a `:` range that denotes one (`SUM(ROW(A1:C3))` = 6 and `COUNT(ROW(A1:C3))` = 3 for the 3x1 `[1,2,3]`, `SUM(ROW(MyName))` = 6 for a name over three rows). An open range is refused there. A reference-returning function normally stays scalar, except `INDEX`: `ROW(INDEX(A1:C3,0,1))` resolves that reference once and yields `[1,2,3]` in array position. Those are the array-entered answers, which is the mode MySheet implements everywhere; typed into Excel, `ROW` of a rectangle is the single top row number, so `SUM(ROW(A1:C3))` is 1 there (measured on Aspose.Cells 26.6.0, 2026-09-10). |
| `ROWS` | `ROWS(range)` | Number of rows in the range. Over a [whole-column/row reference](workbook-and-expressions.md#whole-column-and-whole-row-references) an open row axis uses the populated extent (`ROWS(A:A)` = max − min populated row + 1, 0 if empty — a documented divergence from Excel's fixed grid), a bounded one is exact (`ROWS(1:5)` = 5). A [structured reference](workbook-and-expressions.md#tables) answers its resolved band — `ROWS(Tabela1)` = `3`, the data body, the bare name resolving through the name path like a defined name (pinned). An argument that fails to resolve to a reference reports its own error (`ROWS(NoSuchName)` → `#NAME?`, `ROWS(INDIRECT("zz"))` → `#REF!`); a plain scalar value counts as 1 (a 1x1 array). A **computed array** answers its real extent, through the same gate the folding consumers use: `ROWS(A1:C3*2)` = 3, `ROWS(LEN(A1:A3))` = 3, `ROWS(IF(A1:A3>0,A1:A3))` = 3, `ROWS(ROW(A1:A3))` = 3, and over a [dynamic-array producer](workbook-and-expressions.md#dynamic-array-producers) `ROWS(FILTER(A1:A3,A1:A3>0))` = 2 and `ROWS(SEQUENCE(5))` = 5 — matching Excel typed and array-entered alike (measured on Aspose.Cells 26.6.0, 2026-09-10). A producer's own 1x1 error is reported as itself rather than as a missing reference (`ROWS(FILTER(A1:A3,A1:A3>100))` = `#CALC!`, `ROWS(SEQUENCE(-1))` = `#VALUE!`), while an error ELEMENT inside a rectangle does not hide the shape (`ROWS(SORT(E1:E3))` = 3). |
...
| `UNIQUE` | `UNIQUE(array, [by_col], [exactly_once])` | The distinct rows of `array` (or, with `by_col`, its distinct columns) in order of FIRST appearance, and a [dynamic-array producer](workbook-and-expressions.md#dynamic-array-producers): bare in a cell it shows the top-left, while a consumer reads the whole array (over 9, 5, 9, 0: `SUM` 14, `COUNTA` 3, `ROWS` 3). Two candidates match when every cell pair is the same KIND and the same value: numbers numerically, booleans by value, text ORDINALLY — **case-SENSITIVE**, so "a", "A", "b" keeps all three, while `COUNTIF` and `MATCH` beside it stay case-insensitive (measured; Microsoft's page says nothing about case either way). Kinds never cross (`1`, `"1"`, `TRUE` are three rows), a BLANK is its own key — equal to neither 0 nor `""` nor FALSE — and an ERROR is a key like any other, equal to the same error code. `exactly_once` keeps the candidates occurring ONCE, following Microsoft's page rather than the measured oracle, which keeps the distinct-count shape and **overwrites its front** with the once-occurring values, leaving every distinct element the once-list does not reach where it was — over 1, 1, 2, 3, 3, 4 that answers 2, 4, 3, 4, a four-row result carrying a value that occurs twice (3) and a duplicate of one that does not (4), contradicted by its own row count. Nothing left is `#CALC!`. An open-range `array` is refused with `#VALUE!`. |
| `VLOOKUP` | `VLOOKUP(lookup_value, table_range, col_index_num, [range_lookup])` | Vertical lookup in the first column of a table; exact or approximate. An absent-cell key follows the numeric-zero rule, while an empty-text key matches stored or formula-produced empty text rather than a truly blank cell; exact mode returns `#N/A` when no text candidate exists, and approximate mode selects the last exact-text candidate. A scalar or resolved single-cell table is treated as 1x1 and receives the normal mode/index validation (a miss → `#N/A`). A direct error-valued cell table returns `#N/A` in exact mode before index bounds, while approximate mode propagates its error; a named error cell propagates its error in either mode. |
| `XLOOKUP` | `XLOOKUP(lookup_value, lookup_range, return_range, [if_not_found], [match_mode], [search_mode])` | Modern lookup with not-found fallback and match/search modes. An absent-cell key matches a truly blank lookup cell, while an empty-text key matches stored or formula-produced empty text rather than a truly blank cell. In exact and approximate modes it returns the not-found result when no text candidate exists; with multiple candidates, forward search modes select the first and reverse modes select the last. The lookup array must be one-dimensional. The return array must have the same extent on its search axis (rows for a vertical lookup, columns for a horizontal lookup), otherwise `#VALUE!`; extra cells on the return array's other axis are allowed, and the matching row or column remains available to range-aware consumers. A 1x1 lookup is horizontal, so its return array must have exactly one column. References, LET-bound arrays and defined-name computed arrays follow the same rule and each operand is evaluated once. |
| `XMATCH` | `XMATCH(lookup_value, lookup_range, [match_mode], [search_mode])` | 1-based position with `XLOOKUP`'s modes (0 exact — default, -1 exact-or-smaller, 1 exact-or-larger, 2 wildcard). An absent-cell key matches a truly blank lookup cell, while an empty-text key matches stored or formula-produced empty text rather than a truly blank cell. In exact and approximate modes it returns `#N/A` when no text candidate exists; with multiple candidates, forward search modes select the first and reverse modes select the last. Search modes 1/-1 and 2/-2 are accepted; all scan linearly, and 2/-2 agree with Excel only on sorted data. Over an open range, the position is relative to its declared origin, like `MATCH`. |

## Information (18)

The `IS*` functions inspect the evaluated value without coercion (`ISNUMBER("19")` is `FALSE`) and
never propagate errors — they report on them. Scalar information/error consumers (`ISBLANK`, `ISERR`,
`ISERROR`, `ISLOGICAL`, `ISNA`, `ISNONTEXT`, `ISNUMBER`, `ISTEXT`, `N`, `IFERROR`, and `IFNA`) first apply
row/column-position implicit intersection to a multi-cell reference, regardless of whether it is a literal
range, table column, defined name, open range, or cross-sheet reference.
...
| `ISREF` | `ISREF(value)` | `TRUE` when the argument is a reference (cell/range/union) — a syntactic check, regardless of the value. A [structured reference](workbook-and-expressions.md#tables) answers `TRUE` when it resolves (`ISREF(Tabela1[Valor])`, and the bare `ISREF(Tabela1)` through the name path) and `FALSE` when it does not (`ISREF(Tabela1[#Totals])` over a table with no totals row) — both pinned. |
