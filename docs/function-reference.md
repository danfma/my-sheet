...
| `HLOOKUP` | `HLOOKUP(lookup_value, table_range, row_index_num, [range_lookup])` | Horizontal lookup in the first row of a table; exact or approximate; a scalar table is treated as 1x1 (a miss → `#N/A`), `row_index_num` < 1 → `#VALUE!`, beyond the table → `#REF!`. |
...
| `VLOOKUP` | `VLOOKUP(lookup_value, table_range, col_index_num, [range_lookup])` | Vertical lookup in the first column of a table; exact or approximate; a scalar table is treated as 1x1 (a miss → `#N/A`). |
| `XLOOKUP` | `XLOOKUP(lookup_value, lookup_range, return_range, [if_not_found], [match_mode], [search_mode])` | Modern lookup with not-found fallback and match/search modes. The return range must have the same extent on the lookup range's search axis (rows for a vertical lookup, columns for a horizontal lookup), otherwise `#VALUE!`; extra cells on the return range's other axis are allowed. |
