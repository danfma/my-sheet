...
| `SUMIF` | `SUMIF(range, criteria, [sum_range])` | Sums matching cells. A supplied `sum_range` is resized to `range`'s shape from its top-left (`SUMIF(A1:A3,">0",B1)` reads `B1:B3`); an `IF` selector whose selected branch is a reference is accepted in the range slot. |
| `SUMIFS` | `SUMIFS(sum_range, criteria_range1, criteria1, …)` | Sum under multiple criteria-range pairs; unlike `SUMIF`, every range must already have the same size (`#VALUE!` otherwise). |
...
| `AVERAGEIF` | `AVERAGEIF(range, criteria, [average_range])` | Average of matching cells; a supplied `average_range` is resized to `range`'s shape from its top-left; no numeric match → `#DIV/0!`. |
...
| `HLOOKUP` | `HLOOKUP(lookup_value, table_range, row_index_num, [range_lookup])` | Horizontal lookup in the first row of a table; exact or approximate; a scalar or resolved single-cell table is treated as 1x1 and receives the normal mode/index validation (a miss → `#N/A`), `row_index_num` < 1 → `#VALUE!`, beyond the table → `#REF!`. |
...
| `MATCH` | `MATCH(lookup_value, lookup_range, [match_type])` | 1-based position of a value in a range (`match_type`: 1 approximate ascending — default, 0 exact, -1 approximate descending). An absent-cell key is numeric zero; an empty-text key in either approximate mode matches empty text exactly or returns `#N/A`. Over an open range its position is relative to that range's declared origin, not its populated bounding box: `MATCH("x",$4:$4,0)` is `3` for C4. |
