...
`VLOOKUP` and `HLOOKUP` also accept a scalar or a resolved single cell as a 1x1 table. A direct error-valued
cell table returns `#N/A` in exact mode before index bounds and propagates the error in approximate mode;
