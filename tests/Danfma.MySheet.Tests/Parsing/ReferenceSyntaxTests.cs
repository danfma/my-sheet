...
public class ReferenceSyntaxTests
{
    ...
    public async Task SheetNames_AreCaseInsensitive()
    {
        ...
        await Assert
            .That(
                ExpressionParser.Parse("=SHEET1!A1", other).Evaluate(workbook).AsObject() as double?
            )
            .IsEqualTo(5.0);
    }
    ...
    public async Task SheetNames_AreCaseInsensitive_AfterRoundTrip()
    {
        ...
        await Assert
            .That(
                ExpressionParser.Parse("=sheet1!A1", context).Evaluate(restored).AsObject()
                    as double?
            )
            .IsEqualTo(5.0);
    }
    ...
    public async Task AbsoluteMarkers_AreIgnored()
    {
        ...
        await Assert
            .That(
                ExpressionParser.Parse("=$A1+A$1", sheet).Evaluate(workbook).AsObject() as double?
            )
            .IsEqualTo(14.0);
    }
    ...
    public async Task SheetQualifiedCell()
    {
        ...
        await Assert
            .That(
                ExpressionParser.Parse("=Sheet2!A1", sheet1).Evaluate(workbook).AsObject()
                    as double?
            )
            .IsEqualTo(42.0);
        await Assert
            .That(
                ExpressionParser.Parse("=Sheet2!$A$1", sheet1).Evaluate(workbook).AsObject()
                    as double?
            )
            .IsEqualTo(42.0);
    }
    ...
    public async Task SheetQualifiedRange()
    {
        ...
        await Assert
            .That(
                ExpressionParser.Parse("=SUM(Sheet2!A1:A3)", sheet1).Evaluate(workbook).AsObject()
                    as double?
            )
            .IsEqualTo(6.0);
    }
    ...
    public async Task QuotedSheetName()
    {
        ...
        await Assert
            .That(
                ExpressionParser.Parse("='My Sheet'!B2", sheet1).Evaluate(workbook).AsObject()
                    as double?
            )
            .IsEqualTo(99.0);
    }
    ...
    public async Task UnionOperator_CombinesAreas()
    {
        ...
        await Assert
            .That(
                ExpressionParser.Parse("=SUM((A1:A3,C1:C3))", sheet).Evaluate(workbook).AsObject()
                    as double?
            )
            .IsEqualTo(66.0);
        await Assert
            .That(
                ExpressionParser.Parse("=COUNT((A1,A3,C2))", sheet).Evaluate(workbook).AsObject()
                    as double?
            )
            .IsEqualTo(3.0);
    }
