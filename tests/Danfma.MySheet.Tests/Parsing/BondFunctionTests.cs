...
public class BondFunctionTests
{
    ...
    {
        var expected = Financial
            .CoupPCD(Settlement, Maturity, Frequency.SemiAnnual, DayCountBasis.UsPsa30_360)
            .ToOADate();
        await Assert
            .That(Num(Calc("=COUPPCD(DATE(2011,1,25),DATE(2015,11,15),2)")))
            .IsEqualTo(expected)
            .Within(Tolerance);
    }
    ...
    {
        var expected = Financial
            .CoupNCD(Settlement, Maturity, Frequency.SemiAnnual, DayCountBasis.UsPsa30_360)
            .ToOADate();
        await Assert
            .That(Num(Calc("=COUPNCD(DATE(2011,1,25),DATE(2015,11,15),2)")))
            .IsEqualTo(expected)
            .Within(Tolerance);
    }
    ...
    {
        var expected = Financial.CoupNum(
            Settlement,
            Maturity,
        ...
        await Assert
            .That(Num(Calc("=COUPNUM(DATE(2011,1,25),DATE(2015,11,15),2)")))
            .IsEqualTo(expected)
            .Within(Tolerance);
    }
    ...
    public async Task CoupDays_AcrossBases_MatchExcel()
    {
        ...
        {
            var expected = Financial.CoupDays(
                Settlement,
                Maturity,
                Frequency.SemiAnnual,
                (DayCountBasis)basis
            ...
            await Assert
                .That(Num(Calc($"=COUPDAYS(DATE(2011,1,25),DATE(2015,11,15),2,{basis})")))
                .IsEqualTo(expected)
                .Within(Tolerance);
        }
    }
    ...
    public async Task CoupDayBs_AcrossBases_MatchExcel()
    {
        foreach (var basis in new[] { 0, 1, 2, 3, 4 })
        {
            var expected = Financial.CoupDaysBS(
                Settlement,
                Maturity,
                Frequency.SemiAnnual,
                (DayCountBasis)basis
            ...
            await Assert
                .That(Num(Calc($"=COUPDAYBS(DATE(2011,1,25),DATE(2015,11,15),2,{basis})")))
                .IsEqualTo(expected)
                .Within(Tolerance);
        }
    }
    ...
    public async Task CoupDaysNc_AcrossBases_MatchExcel()
    {
        foreach (var basis in new[] { 0, 1, 2, 3, 4 })
        {
            var expected = Financial.CoupDaysNC(
                Settlement,
                Maturity,
                Frequency.SemiAnnual,
                (DayCountBasis)basis
            ...
            await Assert
                .That(Num(Calc($"=COUPDAYSNC(DATE(2011,1,25),DATE(2015,11,15),2,{basis})")))
                .IsEqualTo(expected)
                .Within(Tolerance);
        }
    }
    ...
    {
        await Assert
            .That(Calc("=COUPNUM(DATE(2015,11,15),DATE(2011,1,25),2)"))
    ...
    {
        await Assert
            .That(Calc("=COUPDAYS(DATE(2011,1,25),DATE(2015,11,15),3)"))
            .IsEqualTo(ErrorValue.Number);
    }
    ...
    {
        await Assert
            .That(Calc("=COUPDAYS(DATE(2011,1,25),DATE(2015,11,15),2,5)"))
            .IsEqualTo(ErrorValue.Number);
    }
    ...
    public async Task Price_MatchesExcel()
    {
        ...
        var expected = Financial.Price(
            new DateTime(2008, 2, 15),
        ...
        await Assert
            .That(Num(Calc("=PRICE(DATE(2008,2,15),DATE(2017,11,15),0.0575,0.065,100,2,0)")))
            .IsEqualTo(expected)
            .Within(1e-6);
    }
    ...
    public async Task Price_AllBases_MatchExcel()
    {
        foreach (var basis in new[] { 0, 1, 2, 3, 4 })
        {
            ...
            await Assert
                .That(
                    Num(Calc($"=PRICE(DATE(2011,1,25),DATE(2015,11,15),0.05,0.06,100,2,{basis})"))
                )
                .IsEqualTo(expected)
                .Within(1e-6);
        }
    ...
    public async Task Yield_MatchesExcel()
    {
        ...
        await Assert
            .That(Num(Calc("=YIELD(DATE(2008,2,15),DATE(2016,11,15),0.0575,95.04287,100,2,0)")))
            .IsEqualTo(expected)
            .Within(1e-6);
    }
    ...
    public async Task PriceYield_InverseAcrossBases()
    {
        ...
        {
            var price = Num(
                Calc($"=PRICE(DATE(2011,1,25),DATE(2015,11,15),0.05,0.06,100,2,{basis})")
            );
            var yield = Num(
                Calc(
                    $"=YIELD(DATE(2011,1,25),DATE(2015,11,15),0.05,{price.ToString(System.Globalization.CultureInfo.InvariantCulture)},100,2,{basis})"
                )
            );
    ...
    public async Task Price_LastPeriod_MatchesExcel()
    {
        ...
        await Assert
            .That(Num(Calc("=PRICE(DATE(2017,7,1),DATE(2017,11,15),0.0575,0.065,100,2,0)")))
            .IsEqualTo(expected)
            .Within(1e-6);
    }
    ...
    {
        var expected = Financial.Duration(
        ...
        await Assert
            .That(Num(Calc("=DURATION(DATE(2008,1,1),DATE(2016,1,1),0.08,0.09,2,1)")))
            .IsEqualTo(expected)
            .Within(1e-6);
    }
    ...
    {
        var expected = Financial.MDuration(
            new DateTime(2008, 1, 1),
            new DateTime(2016, 1, 1),
            0.08,
            0.09,
        ...
        await Assert
            .That(Num(Calc("=MDURATION(DATE(2008,1,1),DATE(2016,1,1),0.08,0.09,2,1)")))
            .IsEqualTo(expected)
            .Within(1e-6);
    }
    ...
    {
        var expected = Financial.AccrInt(
            new DateTime(2008, 3, 1),
            new DateTime(2008, 8, 31),
        ...
        await Assert
            .That(Num(Calc("=ACCRINT(DATE(2008,3,1),DATE(2008,8,31),DATE(2008,5,1),0.1,1000,2,0)")))
            .IsEqualTo(expected)
            .Within(1e-6);
    }
        ...
        foreach (var basis in new[] { 0, 1, 2, 3, 4 })
        {
            ...
            await Assert
                .That(
                    Num(
                        Calc(
                            $"=ACCRINT(DATE(2008,1,1),DATE(2011,7,1),DATE(2010,4,15),0.08,1000,2,{basis})"
                        )
                    )
                )
                .IsEqualTo(expected)
    ...
    {
        var expected = Financial.AccrIntM(
        ...
        await Assert
            .That(Num(Calc("=ACCRINTM(DATE(2008,4,1),DATE(2008,6,15),0.1,1000,3)")))
            .IsEqualTo(expected)
            .Within(1e-6);
    }
    ...
    public async Task Disc_MatchesExcel()
    {
        ...
        await Assert
            .That(Num(Calc("=DISC(DATE(2018,1,25),DATE(2018,6,15),97.975,100,2)")))
            .IsEqualTo(expected)
            .Within(1e-9);
    }
    ...
    {
        var expected = Financial.IntRate(
            new DateTime(2008, 2, 15),
            new DateTime(2008, 5, 15),
            1000000,
            1014420,
            DayCountBasis.Actual360
        );
        await Assert
            .That(Num(Calc("=INTRATE(DATE(2008,2,15),DATE(2008,5,15),1000000,1014420,2)")))
            .IsEqualTo(expected)
            .Within(1e-9);
    }
    ...
    {
        var expected = Financial.Received(
            new DateTime(2008, 2, 15),
            new DateTime(2008, 5, 15),
            1000000,
            0.0575,
        ...
        await Assert
            .That(Num(Calc("=RECEIVED(DATE(2008,2,15),DATE(2008,5,15),1000000,0.0575,2)")))
            .IsEqualTo(expected)
            .Within(1e-6);
    }
    ...
    {
        var expected = Financial.PriceDisc(
            new DateTime(2008, 2, 16),
            new DateTime(2008, 3, 1),
            0.0525,
            100,
            DayCountBasis.Actual360
        ...
        await Assert
            .That(Num(Calc("=PRICEDISC(DATE(2008,2,16),DATE(2008,3,1),0.0525,100,2)")))
            .IsEqualTo(expected)
            .Within(1e-9);
    }
    ...
    {
        var expected = Financial.YieldDisc(
            new DateTime(2008, 2, 16),
            new DateTime(2008, 3, 1),
            99.795,
            100,
        ...
        await Assert
            .That(Num(Calc("=YIELDDISC(DATE(2008,2,16),DATE(2008,3,1),99.795,100,2)")))
            .IsEqualTo(expected)
            .Within(1e-9);
    }
    ...
    {
        var expected = Financial.PriceMat(
            new DateTime(2008, 2, 15),
            new DateTime(2008, 4, 13),
        ...
        await Assert
            .That(
                Num(
                    Calc(
                        "=PRICEMAT(DATE(2008,2,15),DATE(2008,4,13),DATE(2007,11,11),0.061,0.061,2)"
                    )
                )
            )
            .IsEqualTo(expected)
    ...
    {
        var expected = Financial.YieldMat(
            new DateTime(2008, 3, 15),
            new DateTime(2008, 11, 3),
            new DateTime(2007, 11, 8),
            0.0625,
            100.0123,
            DayCountBasis.Actual360
        );
    ...
    {
        var expected = Financial.TBillEq(
            new DateTime(2008, 3, 31),
            new DateTime(2008, 6, 1),
            0.0914
        );
        await Assert
            .That(Num(Calc("=TBILLEQ(DATE(2008,3,31),DATE(2008,6,1),0.0914)")))
            .IsEqualTo(expected)
            .Within(1e-9);
    }
    ...
    {
        var expected = Financial.TBillPrice(
            new DateTime(2008, 3, 31),
            new DateTime(2008, 6, 1),
            0.09
        );
        await Assert
            .That(Num(Calc("=TBILLPRICE(DATE(2008,3,31),DATE(2008,6,1),0.09)")))
            .IsEqualTo(expected)
            .Within(1e-9);
    }
    ...
    {
        var expected = Financial.TBillYield(
            new DateTime(2008, 3, 31),
            new DateTime(2008, 6, 1),
            98.45
        );
        await Assert
            .That(Num(Calc("=TBILLYIELD(DATE(2008,3,31),DATE(2008,6,1),98.45)")))
            .IsEqualTo(expected)
            .Within(1e-9);
    }
    ...
    {
        await Assert
            .That(Calc("=TBILLPRICE(DATE(2008,3,31),DATE(2010,6,1),0.09)"))
            .IsEqualTo(ErrorValue.Number);
    }
    ...
    public async Task Price_SurvivesSaveAndLoad()
    {
        var workbook = new Workbook();
        var sheet = workbook.Sheets.Add("Sheet1");
        sheet["A1"] = ExpressionParser.Parse(
        ...
        try
        {
            ...
            var expectedPrice = Financial.Price(
                new DateTime(2008, 2, 15),
                new DateTime(2017, 11, 15),
                0.0575,
                0.065,
            ...
            await Assert
                ...
                .Within(1e-6);
            await Assert
                .That(Num(loadedSheet["A2"].Evaluate(loaded).AsObject()))
                .IsEqualTo(10)
                .Within(Tolerance);
        }
        finally
