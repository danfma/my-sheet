...
public class FunctionExtensionTests
{
    ...
    public async Task CustomFunction_IsResolvedAndInvoked()
    {
        ...
        workbook.RegisterFunction("DOUBLE", (args, wb) => (args[0].Compute(wb) as double? ?? 0) * 2);

        await Assert.That(ExpressionParser.Parse("=DOUBLE(21)", sheet).Compute(workbook) as double?)
            .IsEqualTo(42.0);
    }
    ...
    public async Task UnregisteredFunction_IsNameError()
    {
        ...
        await Assert.That(ExpressionParser.Parse("=NOPE()", sheet).Compute(workbook)).IsEqualTo(ErrorValue.Name);
    }
    ...
    public async Task XlfnPrefix_IsNormalized()
    {
        ...
        await Assert.That(ExpressionParser.Parse("=XLFN.MYFN()", sheet).Compute(workbook) as double?).IsEqualTo(7.0);
        await Assert.That(ExpressionParser.Parse("=_xlfn.MYFN()", sheet).Compute(workbook) as double?).IsEqualTo(7.0);
    }
    ...
    public async Task UnderscoreInFunctionName_Tokenizes()
    {
        ...
        await Assert.That(ExpressionParser.Parse("=A_HIDE()", sheet).Compute(workbook) as bool?).IsEqualTo(true);
    }
    ...
    public async Task CustomFunctionCall_SurvivesSerialization()
    {
        ...
        restored.RegisterFunction(
            "CUSTOM",
            (args, wb) => (args[0].Compute(wb) as double? ?? 0) + (args[1].Compute(wb) as double? ?? 0));
