...
public class SheetFunctionTests
{
    ...
    public async Task Sheet_ByNameAndCurrentSheet()
    {
        ...
        await Assert.That(ExpressionParser.Parse("=SHEET(\"Sheet2\")", sheet1).Compute(workbook) as double?)
            .IsEqualTo(2.0);
        ...
        await Assert.That(ExpressionParser.Parse("=A1", sheet1).Compute(workbook) as double?).IsEqualTo(1.0);
    }
}
