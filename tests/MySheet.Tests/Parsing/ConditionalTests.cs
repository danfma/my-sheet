...
public class ConditionalTests
{
    ...
    {
        await Assert.That(Calc("=IF(0>1,99)") as bool?).IsEqualTo(false);
    }
    ...
    public async Task If_ShortCircuits_UntakenBranch()
    {
        ...
        await Assert.That(Calc("=IF(A1=0,0,1/A1)", ("A1", new NumberValue(0))) as double?).IsEqualTo(0.0);
    }

    [Test]
    public async Task If_TextCondition()
    {
        await Assert.That(Calc("=IF(A1=\"sim\",1,0)", ("A1", new MySheet.Expressions.StringValue("sim"))) as double?)
            .IsEqualTo(1.0);
    }
    ...
    {
        await Assert.That(Calc("=AND(1>0,2>1)") as bool?).IsEqualTo(true);
    }

    [Test]
    public async Task And_OneFalse()
    {
        await Assert.That(Calc("=AND(1>0,2<1)") as bool?).IsEqualTo(false);
    }

    [Test]
    public async Task Or_OneTrue()
    {
        await Assert.That(Calc("=OR(1<0,2>1)") as bool?).IsEqualTo(true);
    }

    [Test]
    public async Task Not_InvertsCondition()
    {
        await Assert.That(Calc("=NOT(1>0)") as bool?).IsEqualTo(false);
    }
