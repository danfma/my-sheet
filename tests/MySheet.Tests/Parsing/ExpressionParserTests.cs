...
public class ExpressionParserTests
{
    ...
    {
        await Assert.That(Calc("=1<2") as bool?).IsEqualTo(true);
    ...
    public async Task Range_InScalarContext_IsValueError()
    {
        ...
        await Assert.That(ExpressionParser.Parse("=A1:A3", sheet).Compute(workbook)).IsEqualTo(ErrorValue.NotValue);
    }
    ...
    public async Task NestedFunctions()
    {
        ...
        var result = ExpressionParser.Parse("=SUM(SUM(A1,A2),A3)", sheet).Compute(workbook) as double?;
    ...
    public async Task ErrorFromFunction_PropagatesThroughArithmetic()
    {
        ...
        await Assert.That(ExpressionParser.Parse("=A1+FOO()", sheet).Compute(workbook)).IsEqualTo(ErrorValue.Name);
    }
    ...
    public async Task MissingComma_Throws()
    {
        ...
        await Assert.That(() => ExpressionParser.Parse("=SUM(A1 A2)", sheet)).Throws<ParseException>();
    }
    ...
    {
        await Assert.That(Calc("TRUE") as bool?).IsEqualTo(true);
    }
